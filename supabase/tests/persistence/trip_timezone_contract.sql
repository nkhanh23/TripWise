...
('7a000000-0000-4000-8000-000000000001','11111111-1111-4111-8111-111111111111','Timezone','Free text','2028-03-12','2028-03-12');
insert into public.itinerary_days(id,trip_id,day_number,date) values
('7a000000-0000-4000-8000-000000000011','7a000000-0000-4000-8000-000000000001',1,'2028-03-12');
...
do $$
declare v_trip_id uuid := '7a000000-0000-4000-8000-000000000001'; r integer; c jsonb; result jsonb; before_row jsonb; bad text; extra jsonb;
begin
  select workspace_revision,to_jsonb(t) into r,before_row from public.trips t where t.id=v_trip_id;
  c := jsonb_build_object('tripId',v_trip_id,'expectedRevision',r,'timezone','Asia/Ho_Chi_Minh');
  perform pg_temp.tz_assert(before_row->'schedule_timezone'='null'::jsonb,'legacy null');
  perform pg_temp.tz_assert(public.get_saved_trip_detail(v_trip_id)->'timezone'=
  ...
  perform pg_temp.tz_assert(public.get_saved_trip_detail(v_trip_id)->'timezone' = result-'tripId'-'revision','read back');
  perform pg_temp.tz_assert(public.read_trip_progress(jsonb_build_object('tripId',v_trip_id,'kind','state'))->'timezone'
    = result-'tripId'-'revision','progress read back');
  raise notice 'TIMEZONE_RUNTIME_OWNER_SET_READBACK=%',result;
  select to_jsonb(t) into before_row from public.trips t where t.id=v_trip_id;
  ...
  perform pg_temp.tz_assert((select to_jsonb(t)=before_row from public.trips t where t.id=v_trip_id),'stale zero side effects');
  ...
  exception when sqlstate 'TW007' then null; end;
  begin update public.trips set timezone_confirmed_at=now() where public.trips.id=v_trip_id; raise exception 'Raw timestamp accepted';
  exception when sqlstate 'TW013' then null; end;
  begin update public.trips set timezone_provenance='GOOGLE' where public.trips.id=v_trip_id; raise exception 'Raw provenance accepted';
  exception when sqlstate 'TW013' then null; end;
  begin update public.trips set schedule_timezone='Europe/London' where public.trips.id=v_trip_id; raise exception 'Raw timezone accepted';
  exception when sqlstate 'TW013' then null; end;
  begin
  ...
  exception when sqlstate 'TW013' then null; end;
  perform pg_temp.tz_assert((select to_jsonb(t)=before_row from public.trips t where t.id=v_trip_id),'invalid zero side effects');
  ...
  update public.trips set destination='New destination' where public.trips.id=v_trip_id;
  perform pg_temp.tz_assert((select schedule_timezone is null and timezone_provenance is null
    and timezone_confirmed_at is null and workspace_revision=r+1 from public.trips t where t.id=v_trip_id),'destination invalidation');
  perform pg_temp.tz_assert((select count(*)=0 from public.trip_progress_events where trip_id=v_trip_id),'no lifecycle events');
end $$;
select set_config('request.jwt.claim.sub','22222222-2222-4222-8222-222222222222',true);
do $$ begin
  begin perform public.set_trip_timezone('{"tripId":"7a000000-0000-4000-8000-000000000001","expectedRevision":1,"timezone":"Asia/Tokyo"}');
    raise exception 'Foreign accepted'; exception when sqlstate 'TW008' then null; end;
  perform pg_temp.tz_assert(public.get_saved_trip_detail('7a000000-0000-4000-8000-000000000001') is null,'foreign read');
...
reset role;
do $$ begin
  begin update public.trips set schedule_timezone='Asia/Tokyo' where id='7a000000-0000-4000-8000-000000000001';
