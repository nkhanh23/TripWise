...
-- This function intentionally does not implement move/reorder; that persistence
-- operation remains owned by FEATURE-P2-T002 and must be a separate atomic CAS command.
...
begin
  ...
  if p_command is null or octet_length(p_command::text) > 50000 or jsonb_typeof(p_command) <> 'object'
     or not (p_command - 'type' - 'tripId' - 'itemId' - 'expectedRevision' - 'patch' - 'status' - 'links' = '{}'::jsonb) then
  ...
  if v_type not in ('update_item', 'transition_item_status', 'replace_source_links')
     or v_trip_id is null or v_item_id is null or v_expected_revision is null or v_expected_revision < 1 then
    raise exception 'Workspace command is invalid.' using errcode = 'TW007';
  end if;

  -- The trip-row lock serializes all workspace commands for one owner trip.
  ...
  if v_type = 'update_item' then
    ...
    if jsonb_object_length(v_patch) = 0
       or not (v_patch - 'kind' - 'placeName' - 'placeQuery' - 'flexibility' - 'priority' - 'startTime' - 'endTime' - 'note' - 'contact' - 'transport' - 'accommodation' = '{}'::jsonb) then
    ...
    v_final := jsonb_populate_record(v_original, jsonb_build_object(
      'item_kind', case when v_patch ? 'kind' then v_patch->'kind' else to_jsonb(v_original.item_kind) end,
