...
const valid = { googlePlaceId: 'id', name: 'Singapore', formattedAddress: 'Singapore', latitude: 1.35, longitude: 103.82 };
...
Deno.test('valid request trims query and bounds filtered output', async () => { let query = ''; const response = await run(request('{"query":" Oslo "}'), async (value) => { query = value; return [...Array(11).fill(valid), { ...valid, googlePlaceId: '' }]; }); assertEquals(query, 'Oslo'); assertEquals(response.status, 200); assertEquals((await response.json()).data.length, 10); });
