...
text = text.replace(
  /onPress=\{onPress\}\n\s*style=\{styles\.nativeMarkerWrap\}>/,
  `onPress={dimmed ? undefined : onPress}
      disabled={dimmed}
      style={[styles.nativeMarkerWrap, dimmed && { opacity: 0.5 }]}>`
);

// 5. ClusterPin signature & props
text = text.replace(
...
// 6. ExploreMapCanvas component signature
text = text.replace(
  /export const ExploreMapCanvas = memo\(function ExploreMapCanvas\(\{[\s\S]*?onRegionChangeComplete,\n\}: Props\) \{/,
...
// 7. hooks and state tracking
const hooksReplacement = `  const [currentRegion, setCurrentRegion] = useState<ExploreMapRegion>(INITIAL_EXPLORE_REGION);
...
// 9. Render Hints and pass dimmed to Pins
// Wait, we need to replace the validMarkerCoordinates.map contents too
// Let's replace the whole {validMarkerCoordinates.map(...) section

const mapReplacement = `{hints.map((hint) => (
          <Marker
...
text = text.replace(
  /\{validMarkerCoordinates\.map\([\s\S]*?<\/MapView>/,
  mapReplacement
);

// 10. export buildExplorationHints and details
...
console.log('Fixed ExploreMapCanvas');
