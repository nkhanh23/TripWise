...
const pages = await (await fetch('http://localhost:8084/json/list')).json();
const page = pages.find(p => p.appId === 'com.anonymous.tripwisemobile');
if (!page) throw new Error('Android runtime unavailable');
const socket = new WebSocket(page.webSocketDebuggerUrl.replace('localhost', '127.0.0.1'), { origin: 'http://localhost:8084' });
...
void send('Runtime.runIfWaitingForDebugger', {});
