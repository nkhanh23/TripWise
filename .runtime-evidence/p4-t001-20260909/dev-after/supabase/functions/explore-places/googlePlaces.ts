...
export function parseGooglePlacesResponse(value: unknown, requestedCategory: ExploreCategory): ExplorePlaceResult[] {
  ...
  return value.places.map((raw) => {
    if (!isRecord(raw) || typeof raw.id !== 'string' || !/^[A-Za-z0-9_-]{10,200}$/.test(raw.id)
      || !isRecord(raw.displayName) || typeof raw.displayName.text !== 'string' || !raw.displayName.text.trim() || raw.displayName.text.length > 200
      ...
      || !Array.isArray(raw.types) || raw.types.length > 50 || raw.types.some((type) => typeof type !== 'string' || type.length > 100)) {
      throw new ExplorePlacesError('EXPLORE_PROVIDER_INVALID_RESPONSE', 'Place discovery returned invalid data.', 502);
    }
    if (raw.primaryType !== undefined && (typeof raw.primaryType !== 'string' || raw.primaryType.length > 100)) {
      throw new ExplorePlacesError('EXPLORE_PROVIDER_INVALID_RESPONSE', 'Place discovery returned invalid data.', 502);
    }
    ...
    if (raw.formattedAddress !== undefined && (typeof raw.formattedAddress !== 'string' || raw.formattedAddress.length > 500)
      || raw.rating !== undefined && (typeof raw.rating !== 'number' || !Number.isFinite(raw.rating) || raw.rating < 0 || raw.rating > 5)
      || raw.userRatingCount !== undefined && (!Number.isSafeInteger(raw.userRatingCount) || (raw.userRatingCount as number) < 0)) {
      throw new ExplorePlacesError('EXPLORE_PROVIDER_INVALID_RESPONSE', 'Place discovery returned invalid data.', 502);
    }
    return {
...
export async function discoverGooglePlaces(
  ...
  try {
    ...
    if (controller.signal.aborted) throw new DOMException('Cancelled', 'AbortError');
    const places = parseGooglePlacesResponse(payload, request.category);
    if (places.length > (request.limit ?? 12) || new Set(places.map((place) => place.googlePlaceId)).size !== places.length) {
      throw new ExplorePlacesError('EXPLORE_PROVIDER_INVALID_RESPONSE', 'Place discovery returned invalid data.', 502);
    }
    return places;
  } catch (error) {
