// Evidence harness only. No production module edits or credential output/storage.
...
async function main() {
  ...
  if (equivalence.length !== 7 || equivalence.some(item => !item.Identical)) throw Error('EDGE_EQUIVALENCE_REQUIRED');
  const env = {};
  ...
  const { SupabaseCandidateDiscoveryRepository } = load(path.join(root, 'mobile/src/integration/remote/supabaseCandidateDiscoveryRepository.ts'));
  const { CandidateDiscoverySession } = load(path.join(root, 'mobile/src/integration/candidateDiscoverySession.ts'));
  const { mapCandidateRankingInput } = load(path.join(root, 'mobile/src/integration/candidateDiscoveryContract.ts'));
  write('live-loaded-source-hashes.json', moduleHashes);
  const repository = new SupabaseCandidateDiscoveryRepository(client);
  const scope = new CandidateDiscoverySession(repository);
  const request = { center: { latitude: 13.7437, longitude: 100.4888 }, radiusMeters: 1000, category: 'attractions', limit: 3 };
  ...
  try {
    phase = 'discovery';
    const candidates = await scope.discover(request);
    const ranking = mapCandidateRankingInput(candidates, { origin: request.center, preferredCategories: ['attractions'] });
    write('live-smoke-sanitized-response.json', { count: candidates.length, candidates, rankingInputVersion: ranking.version, review: ranking.review });
    if (!candidates.length) throw Error('EMPTY_REQUIRES_REVIEW');
    phase = 'cancellation';
    const cancellation = scope.discover(request);
    scope.cancel();
    let resultReturned = false;
    let code;
    try { await cancellation; resultReturned = true; } catch (error) { code = error.code; }
    write('live-cancellation.json', { resultReturned, code, method: 'immediate cancel after dispatch', serverReceiptProven: false });
  ...
  console.log('LIVE_CANDIDATE_SMOKE_PASS; owner data unchanged; credentials not emitted');
}
main().catch(error => {
  const allowed = ['EDGE_EQUIVALENCE_REQUIRED', 'PROJECT_MISMATCH', 'PUBLIC_CONFIG_MISSING', 'NORMAL_DEV_CREDENTIALS_UNAVAILABLE', 'NORMAL_AUTH_FAILED', 'OWNER_SNAPSHOT_INCOMPLETE', 'EMPTY_REQUIRES_REVIEW', 'CANCELLATION_RESULT_UNEXPECTED', 'OWNER_DATA_CHANGED_STOP'];
  const code = allowed.includes(error.message) ? error.message : (['cancelled', 'unauthorized', 'providerUnavailable', 'rateLimited', 'invalidResponse', 'timeout'].includes(error.code) ? error.code : 'HARNESS_FAILED_SAFE');
