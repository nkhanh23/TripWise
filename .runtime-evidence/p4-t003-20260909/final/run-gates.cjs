...
const gates = group === 'edge' ? [
  ...
  ['edge-test','deno',['test','supabase/functions/discover-events/']],
...
] : group === 'doctor' ? [['doctor','npx',['expo-doctor']]] : [
  ['lint','npm',['run','lint']], ['typecheck','npm',['run','typecheck']],
  ['focused','npm',['test','--','--runInBand','event-intelligence.test.ts']],
...
for (const [name,cmd,args] of gates) {
  ...
  fs.writeFileSync(path.join(__dirname,group+'-gates.json'),JSON.stringify(results,null,2));
