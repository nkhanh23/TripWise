...
async function main() {
 ...
 write('auth-precheck.json', { status: auth.error?.status, code: auth.error?.code, name: auth.error?.name, hasSession: Boolean(auth.data.session), role: auth.data.user?.role, anonymous: auth.data.user?.is_anonymous });
 ...
 city: 'London',
 countryCode: 'GB',
 startDateTime: new Date(Date.now() + 86400000).toISOString().replace(/\.\d{3}Z$/, 'Z'),
 endDateTime: new Date(Date.now() + 7 * 86400000).toISOString().replace(/\.\d{3}Z$/, 'Z'),
 limit: 3,
 });
 ...
 await cachedEventRepo.discoverWithFreshness(queryLondon);
 const eventTransportCalls = network.filter(n => n.path === '/functions/v1/discover-events').length;
 if (eventTransportCalls !== 1) throw new Error('DUPLICATE_EVENT_CALL');
 const providerIdentityValid = sanitizedEvents.every((e) => e.provider === 'ticketmaster');
 ...
 attributionCompliance: 'NOT_ESTABLISHED_BY_PROVIDER_SMOKE; see branding-review.md',
 eventTransportCalls,
 repeatedIdenticalRequests: 2,
 ...
 openingHours: placeResult.data.openingHours,
 provenance: placeResult.data.provenance,
 ...
 networkMutationAttempts: network.filter(n => n.path.startsWith('/rest/')).length,
 uncommittedSaves: 0,
 itineraryWrites: 0,
 },
...
main().catch((err) => {
 console.error('LIVE_REVIEW_SMOKE_FAILED:', secretValues.reduce((m, v) => v ? m.split(v).join('[REDACTED]') : m, String(err.message)));
 process.exitCode = 1;
});
