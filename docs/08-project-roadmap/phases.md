...
- ✅ Gradle Wrapper đã có (gradlew, gradlew.bat, gradle-wrapper.jar/properties)
...
- ✅ `./gradlew bootRun --args="--spring.profiles.active=local"` start OK trên port 8080
...
3. Sau mỗi phase phải:
   - Chạy test: `./gradlew clean test` (backend) hoặc test tương ứng
...
- [ ] Phase 3.8 - Refresh token rotation
- [ ] Phase 3.9 - Logout endpoint
- [ ] Phase 3.10 - Auth integration tests
- [ ] Phase 3.11 - Rate limiting cho auth endpoints
...
### H. OSRM Routing
- [ ] Phase 8.1 - OSRM HTTP client
- [ ] Phase 8.2 - Route cache repository
- [ ] Phase 8.3 - Route calculation use case
- [ ] Phase 8.4 - Integrate routing vào itinerary generation
- [ ] Phase 8.5 - Route API endpoint
...
- Kiểm tra không có `backend/.gradle/`, `backend/build/`, `backend/bin/` trong Git tracking
- Kiểm tra `backend/gradle.properties` không chứa đường dẫn local (nếu có thì thêm vào .gitignore hoặc xóa)
...
- `cd` (xóa)
- `backend/gradle.properties` (kiểm tra/xóa nếu chứa local path)
...
### How to verify
```bash
git ls-files | findstr /i "\.gradle build\.gradle\.properties"
# Không nên có .gradle/ hoặc build/ directories
# gradle.properties chỉ OK nếu không chứa local path
...
Xóa file rác "cd" khỏi repo. Kiểm tra không có backend/.gradle/, backend/build/ trong Git tracking.
Kiểm tra backend/gradle.properties không chứa local path. Tạo .env.example nếu chưa có.
...
- `backend/build.gradle` (nếu cần upgrade Flyway)
...
### What will be done
- Uncomment MapStruct dependencies trong `build.gradle`
...
### Files/Folders likely changed
- `backend/build.gradle` (uncomment/thêm MapStruct deps)
...
Uncomment MapStruct trong build.gradle. Thêm lombok-mapstruct-binding.
...
### What will be done
- Thêm Testcontainers dependencies vào build.gradle
...
### Files/Folders likely changed
- `backend/build.gradle` (thêm Testcontainers deps)
...
### What will be done
- Thêm JWT dependency (jjwt hoặc spring-security-oauth2-jose) vào build.gradle
...
### Files/Folders likely changed
- `backend/build.gradle` (thêm JWT dependency)
...
### Files/Folders likely changed
- `backend/build.gradle` (thêm rate limit dependency)
...
### What will be done
- Thêm Hibernate Spatial dependency (hibernate-spatial) vào build.gradle
...
### Files/Folders likely changed
- `backend/build.gradle` (thêm hibernate-spatial)
...
### Files/Folders likely changed
- `backend/build.gradle` (thêm deps nếu cần)
...
### Files/Folders likely changed
- `backend/build.gradle` (thêm springdoc dep)
...
### What will be done
- Multi-stage Dockerfile: build stage (Gradle) → runtime stage (JRE 21)
- Minimal image size
- Non-root user
- Health check
...
### Risks
- Gradle cache in Docker layer — optimize
- JRE image size — use slim/alpine variant

### Suggested prompt
```
Đọc AGENTS.md. Thực hiện Phase 14.1 - Dockerfile backend.
Multi-stage: Gradle build → JRE 21 runtime. Non-root user. Health check. Optimize layers.
```
...
- On PR to develop/main: checkout → setup JDK 21 → Gradle build → test → report
...
Build + test on PR. JDK 21. Gradle cache. Testcontainers support.
```
...
### What will be done
- Run dependency check (OWASP dependency-check hoặc Gradle plugin)
- Fix critical vulnerabilities
- Setup automated scan in CI

### Files/Folders likely changed
- `backend/build.gradle` (plugin)
