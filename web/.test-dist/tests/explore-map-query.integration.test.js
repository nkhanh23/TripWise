...
import { HO_CHI_MINH_VIEWPORT_PRESET, buildExploreMarkerParams, buildExplorePlaceSearchParams, DEFAULT_EXPLORE_PLACE_GROUP, filterMarkersByVisiblePlaces, MAP_MARKER_FETCH_DEBOUNCE_MS, resolveLocationViewport, VIETNAM_BOUNDS, } from "../src/components/explore/explore-map-query.js";
...
test("Explore sends explicit placeType values for every chip state", () => {
    const groups = ["ALL", "ATTRACTION", "FOOD", "HOTEL", "SERVICE"];
...
test("Explore marker sync removes markers that are not present in the current list page", () => {
    ...
    assert.deepEqual(filtered.map((marker) => marker.id), [101, 202]);
});
