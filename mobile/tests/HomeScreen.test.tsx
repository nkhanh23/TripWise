...
} from "../src/features/home/data/mockHome";
...
describe("HomeScreen", () => {
  ...
  it("renders HomeScreen populated state with upcoming trip, quick actions, drafting, and saved section", async () => {
    await render(
      <HomeScreen customData={mockHomePopulatedData} initialStatus="ready" />,
    );
  ...
  it("renders HomeScreen empty state (no upcoming trip)", async () => {
    ...
    expect(
      screen.getByText(
        "Create a trip to start organizing your travel plans, flights, and accommodations.",
      ),
    ).toBeTruthy();
  ...
  it("handles navigation actions from cards and quick actions", async () => {
    ...
    // Tap View Itinerary
    await user.press(
      screen.getByLabelText("View Itinerary: Kyoto Autumn Retreat"),
    );
    ...
    await user.press(
      screen.getByLabelText("Cafe Anthracite, Seoul, South Korea"),
    );
    expect(onNavigatePlaceDetailMock).toHaveBeenCalledWith(
      "saved_cafe_anthracite",
    );
  });
  ...
  it("uses the injected production repository instead of fixture content by default", async () => {
    ...
    await render(
      <HomeScreen repository={repository as SavedTripsRepository} />,
    );
