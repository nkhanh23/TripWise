...
function ThemeConsumer() {
  const { themePreference, effectiveTheme, colors, setThemePreference } = useTheme();
  ...
  return (
    <View testID="theme-root">
      ...
      <Pressable onPress={() => setThemePreference('dark')} testID="set-dark-btn">
        <Text>Set Dark</Text>
      </Pressable>
      <Pressable onPress={() => setThemePreference('light')} testID="set-light-btn">
        <Text>Set Light</Text>
      </Pressable>
    </View>
  );
...
describe('FE-CROSSCUT-001: Theme Foundation', () => {
  it('provides light palette by default when preference is light', async () => {
    ...
    expect(getByTestId('bg-color').props.children).toBe(lightPalette.background.canvas);
    expect(getByTestId('text-color').props.children).toBe(lightPalette.text.primary);
  });
  ...
  it('switches between light and dark themes dynamically without error', async () => {
    ...
    await waitFor(() => {
      ...
      expect(getByTestId('bg-color').props.children).toBe(darkPalette.background.canvas);
      expect(getByTestId('text-color').props.children).toBe(darkPalette.text.primary);
    });
  });
  ...
  it('correctly builds React Navigation 7 theme object with fonts', () => {
    ...
    expect(navThemeLight.colors.background).toBe(lightPalette.background.canvas);
...
describe('FE-CROSSCUT-001: Localization Foundation', () => {
  ...
  it('switches to Vietnamese dynamically and translates accurately', async () => {
    ...
    await waitFor(() => {
      ...
      expect(getByTestId('login-title').props.children).toBe('Chào mừng trở lại');
    });
  });
  ...
  it('interpolates parameters correctly in translation strings', async () => {
    ...
    expect(getByTestId('interpolated').props.children).toBe('Creating your Tokyo trip...');
  });
