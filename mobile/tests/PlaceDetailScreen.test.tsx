...
describe("PlaceDetailScreen", () => {
  ...
  it("renders place details with title, rating, tags, gallery, about, opening hours, tickets, and reviews", async () => {
    ...
    expect(
      screen.getByText(/landmark Buddhist temple on the west bank/),
    ).toBeTruthy();
  ...
  it("renders place not found fallback when placeId does not exist", async () => {
    ...
    await render(
      <PlaceDetailScreen navigation={mockNavigation} route={route} />,
    );

    expect(screen.getByText("Place not found")).toBeTruthy();
    expect(
      screen.getByText("The requested place does not exist in our directory."),
    ).toBeTruthy();
  ...
  it("renders TWPlaceCard component and handles press callback", async () => {
    ...
    await user.press(
      screen.getByLabelText(
        "The Grand Palace, Historical Landmark, đánh giá 4.7 sao",
      ),
    );
  ...
  it("does not resolve fixture details for a production place identity", async () => {
    const route: any = {
      params: { placeId: "1e9a8320-2222-4fcc-9999-999999999999" },
    };
    const getMockPlaceDetailSpy = jest.spyOn(
      placeDetailFixtures,
      "getMockPlaceDetail",
    );
