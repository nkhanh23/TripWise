...
describe("Place Photos Integration & Validation", () => {
  ...
  describe("validateGetPlacePhotoRequest", () => {
    ...
    it("throws ContractValidationError for invalid place ID or invalid maxWidth", () => {
      expect(() =>
        validateGetPlacePhotoRequest({ googlePlaceId: "short" }),
  ...
  describe("parseGetPlacePhotoSuccess", () => {
    it("parses valid photo response with attribution", () => {
      ...
      expect(parsed.data.photoUri).toBe(
        "https://lh3.googleusercontent.com/places/test.jpg",
      );
    ...
    it("throws on invalid envelope", () => {
      expect(() => parseGetPlacePhotoSuccess({ data: {} })).toThrow(
        ContractValidationError,
      );
    });
  });
  ...
  describe("mapPlacePhotoError", () => {
    it("maps error codes cleanly without leaking secrets", () => {
      expect(mapPlacePhotoError({ error: { code: "FORBIDDEN" } }).code).toBe(
        "forbidden",
      );
      expect(
        mapPlacePhotoError({ error: { code: "PHOTO_NOT_FOUND" } }).code,
      ).toBe("notFound");
      expect(
        mapPlacePhotoError({ error: { code: "PHOTO_PROVIDER_RATE_LIMITED" } })
          .code,
      ).toBe("rateLimited");
      expect(
        mapPlacePhotoError({ error: { code: "PHOTO_PROVIDER_AUTH" } }).code,
      ).toBe("providerUnavailable");
      expect(mapPlacePhotoError(new TypeError("Failed to fetch")).code).toBe(
        "network",
      );
    });
  });
  ...
  describe("SupabasePlacePhotoRepository", () => {
    it("fetches photo from edge function and caches it for subsequent calls", async () => {
      ...
      const photo1 = await repo.getPhoto({
        googlePlaceId: "ChIJaSv_6gaZ4jARnbiUVn6Z_YY",
        maxWidth: 1200,
      });
      expect(photo1.photoUri).toBe(
        "https://lh3.googleusercontent.com/places/wat_arun.jpg",
      );
      expect(invokeMock).toHaveBeenCalledTimes(1);

      // Call 2: hits in-memory cache without calling edge function again
      const photo2 = await repo.getPhoto({
        googlePlaceId: "ChIJaSv_6gaZ4jARnbiUVn6Z_YY",
        maxWidth: 1200,
      });
      expect(photo2.photoUri).toBe(
        "https://lh3.googleusercontent.com/places/wat_arun.jpg",
      );
      expect(invokeMock).toHaveBeenCalledTimes(1);
    });
