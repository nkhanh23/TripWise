...
describe("RegisterScreen real-auth composition", () => {
  ...
  it("validates input before calling the repository", async () => {
    const user = userEvent.setup();
    await render(
      <RegisterScreen navigation={navigation} route={{} as never} />,
    );
    await user.press(screen.getByLabelText("Tạo tài khoản"));
  ...
  it.each([false, true])(
    "handles nullable signup session; confirmationRequired=%s",
  ...
  it.each([
    ...
    "maps %s failures to safe localized copy",
  ...
  it("prevents repeated submission while public signup is pending", async () => {
    let resolveSignUp:
      ((value: { confirmationRequired: boolean }) => void) | undefined;
