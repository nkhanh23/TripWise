...
describe("integration DTO validation", () => {
  it("accepts exact non-negative Profile statistics and rejects malformed values", () => {
    expect(
      parseProfileStatistics({ trips_count: 2, saved_places_count: 3 }),
    ).toEqual({
      tripsCount: 2,
      savedPlacesCount: 3,
    });
    ...
    expect(() =>
      parseProfileStatistics({ trips_count: 2, saved_places_count: -1 }),
    ).toThrow();
    expect(() =>
      parseProfileStatistics({
        trips_count: 2,
  ...
  it("accepts a valid generate-trip success and rejects malformed ordering", () => {
        ...
        days: [
          {
            dayNumber: 1,
            date: "2026-09-01",
            items: [{ position: 1, placeName: "Hòn Chồng" }],
          },
        ],
      },
    };
    ...
    expect(() =>
      parseGenerateTripSuccess({
        ...
        data: {
          ...valid.data,
          days: [{ ...valid.data.days[0], dayNumber: 2 }],
        },
      }),
    ).toThrow();
  });
  ...
  it("accepts unresolved nullable coordinate pairs and rejects half-pairs", () => {
    ...
    expect(
      validatePersistTripCommand(base).graph.days[0].items[0],
      ...
      validatePersistTripCommand({
        ...
        graph: {
          ...
          days: [
            {
              ...base.graph.days[0],
              items: [{ position: 1, placeName: "Đại Nội", latitude: null }],
            },
          ],
        },
      }),
  ...
  it("requires protected provenance before accepting provider fields", () => {
    ...
    const detail = {
      ...common,
      days: [
        { id: dayId, dayNumber: 1, date: "2026-09-01", items: [unresolved] },
      ],
    };
    ...
    expect(() =>
      parseSavedTripDetail({
        ...
        days: [
          {
            ...detail.days[0],
            items: [{ ...unresolved, googlePlaceId: "provider-looking-id" }],
          },
        ],
      }),
  ...
  it("validates keyset cursor, limit and saved page response", () => {
    ...
    expect(validateSavedTripsPageRequest({ limit: 50, cursor })).toEqual({
      limit: 50,
      cursor,
    });
