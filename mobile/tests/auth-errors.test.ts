...
describe('Supabase auth error mapping', () => {
  ...
  ])('maps provider code %s to semantic code %s', (remoteCode, semanticCode) => {
    expect(mapAuthError({ code: remoteCode }).code).toBe(semanticCode);
  });

  it('maps semantic errors to centralized translation keys', () => {
    expect(authErrorTranslationKey(new IntegrationError('invalidCredentials'))).toBe('auth.errors.invalidCredentials');
    expect(authErrorTranslationKey(new Error('raw provider internals'))).toBe('auth.errors.unknown');
  });
});
