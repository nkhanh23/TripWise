...
describe("CreateTripWizardScreen", () => {
  ...
  it("renders Step 1 (Destination) with search bar and popular destinations", async () => {
    ...
    expect(
      screen.getByPlaceholderText("Search city, e.g. Bangkok, Tokyo..."),
    ).toBeTruthy();
  ...
  it("shows validation error if destination is cleared and user clicks continue", async () => {
    ...
    expect(
      screen.getByText("Please select or enter a destination for your trip."),
    ).toBeTruthy();
  });
  ...
  it("maps the summary to the repository and renders the validated generated preview", async () => {
    ...
    expect(
      screen.getByText("Start adding places or explore recommendations."),
    ).toBeTruthy();
    ...
    expect(generate).toHaveBeenCalledWith(
      {
        ...
        notes:
          "Travel pace: moderate; budget tier: moderate; group type: couple.",
      },
  ...
  it("preserves input, exposes a safe retry, and prevents concurrent generation requests", async () => {
    ...
    const generate = jest
      ...
      .mockResolvedValueOnce({
        ...
        days: Array.from({ length: 6 }, (_, index) => ({
          ...
          date: `2026-10-${String(15 + index).padStart(2, "0")}`,
          items: [
            { position: 1, placeName: `Bangkok suggestion ${index + 1}` },
          ],
        })),
      });
    await render(
    ...
    expect(
      await screen.findByText(
        "Generation timed out. You can retry once you are ready.",
      ),
    ).toBeTruthy();
