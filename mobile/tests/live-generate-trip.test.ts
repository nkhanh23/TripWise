...
import { mapGeneratedTripToPlannerPreview, mapWizardStateToGenerateTripRequest } from '../src/features/planner/generation';
...
liveTest('authenticated generate-trip validates, maps, and makes zero trip writes', async () => {
  const url = process.env.EXPO_PUBLIC_SUPABASE_URL;
  const publishableKey = process.env.EXPO_PUBLIC_SUPABASE_PUBLISHABLE_KEY;
  const serviceRoleKey = process.env.SUPABASE_SERVICE_ROLE_KEY;
  if (!url || !publishableKey || !serviceRoleKey) throw new Error('Live smoke configuration is missing.');
  ...
  try {
    ...
    expect(preview.days.every((day, dayIndex) => day.dayNumber === dayIndex + 1
      && day.items.every((item, itemIndex) => item.position === itemIndex + 1 && item.resolution === 'UNRESOLVED'))).toBe(true);
