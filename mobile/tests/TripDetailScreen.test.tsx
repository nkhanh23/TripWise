...
describe("TripDetailScreen", () => {
  ...
  it("switches days and updates displayed itinerary items", async () => {
    ...
    expect(
      screen.getByText("ICONSIAM & SookSiam Floating Market"),
    ).toBeTruthy();
  ...
  it("renders empty day state when selected day has no items", async () => {
    ...
    expect(
      screen.getByText(
        "There are no scheduled activities for Day 6 • Oct 17 yet.",
      ),
    ).toBeTruthy();
      ...
      getPhoto: jest.fn().mockImplementation(async ({ googlePlaceId }: any) => {
        ...
        if (googlePlaceId === "place_candidate_2") {
          return {
            googlePlaceId,
            photoUri:
              "https://lh3.googleusercontent.com/places/grand_palace.jpg",
          };
        }
        return { googlePlaceId, photoUri: null };
      }),
  ...
  it("resolves an exact place image without waiting for the slower hero fallback chain", async () => {
    ...
    expect(
      await screen.findByText("Commons author · CC BY-SA 4.0"),
    ).toBeTruthy();
    ...
    await act(async () => {
      finishHero?.({
        ...
        attribution: {
          ...
          license: "CC BY 4.0",
          sourceUrl:
            "https://commons.wikimedia.org/wiki/File:Bangkok_cover.jpg",
        },
      });
      await heroPromise;
    });
    expect(
      await screen.findByText("Destination author · CC BY 4.0"),
    ).toBeTruthy();
  });

  it("starts independent itinerary image resolutions with bounded concurrency", async () => {
    const getPlaceImage = jest.fn(
      () => new Promise<ResolvedImage>(() => undefined),
    );
    const trip: any = {
      ...
      days: [
        {
          ...
          items: [
            ...
            },
            {
              ...
              time: "11:00",
              resolution: "VERIFIED",
              googlePlaceId: "place_grand_palace",
              iconName: "account-balance",
            },
          ],
        },
