...
describe("Supabase auth repository integration with mocked transport", () => {
  it("normalizes login input, maps the session, and exposes no token fields", async () => {
    ...
    const result = await new SupabaseAuthRepository(client).signIn(
      " Owner@Example.COM ",
      "not-logged",
    );
  ...
  it("supports both immediate-session and verification-required registration", async () => {
    ...
    await expect(
      repository.signUp(" Remote Owner ", " OWNER@example.com ", "not-logged"),
    ).resolves.toMatchObject({
      confirmationRequired: false,
    });
    await expect(
      repository.signUp("Remote Owner", "owner2@example.com", "not-logged"),
    ).resolves.toEqual({
      session: null,
    ...
    expect(signUp).toHaveBeenNthCalledWith(1, {
      email: "owner@example.com",
      password: "not-logged",
      options: { data: { display_name: "Remote Owner" } },
    });
  });
  ...
  it("maps auth failures semantically and invokes reset/sign-out methods exactly once", async () => {
    ...
    const failing = new SupabaseAuthRepository({
      auth: {
    ...
    await expect(
      failing.signIn("owner@example.com", "not-logged"),
    ).rejects.toMatchObject<Partial<IntegrationError>>({
      code: "invalidCredentials",
    });
    const repository = new SupabaseAuthRepository({
      auth: { resetPasswordForEmail, signOut },
    } as unknown as SupabaseClient<Database>);
...
describe("Supabase profile repository integration with mocked transport", () => {
  ...
  it("reads the authenticated owner profile through the typed select boundary", async () => {
    ...
    await expect(
      repository.getOwnProfile(asUserId(userId)),
    ).resolves.toMatchObject({
      id: userId,
      displayName: "Remote Owner",
      avatarUrl: null,
  ...
  it("updates reviewed profile fields, including a blank-safe home country, and returns validated remote state", async () => {
    const single = jest.fn().mockResolvedValue({
      data: { ...row, display_name: "Updated", home_country: "VN" },
      error: null,
    });
    ...
    await expect(
      repository.updateOwnProfile(asUserId(userId), {
        displayName: " Updated ",
        homeCountry: " VN ",
        avatarUrl: null,
      }),
