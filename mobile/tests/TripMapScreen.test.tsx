...
describe('TripMapScreen (FE-P12-T001)', () => {
  ...
  it('renders trip map with header, day selector chips, numbered markers, and place preview card', async () => {
    ...
    await renderWithProviders(<TripMapScreen navigation={mockNavigation} route={route} />);
    ...
    expect(screen.getByLabelText(/Selected location 1: Breakfast at Ro Roast/)).toBeTruthy();
    expect(screen.getByLabelText(/Location 2: Wat Arun/)).toBeTruthy();
    expect(screen.getByLabelText(/Location 3: Lunch at Supanniga/)).toBeTruthy();
  ...
  it('filters markers when switching days and renders empty state on day with no items', async () => {
    ...
    // Day 2 has 3 items
    expect(screen.getByLabelText(/Selected location 1: Factory Coffee Barista Brunch/)).toBeTruthy();
    expect(screen.getByLabelText(/Location 2: ICONSIAM & SookSiam Floating Market/)).toBeTruthy();
    expect(screen.getByLabelText(/Location 3: Dinner at Thip Samai Pad Thai/)).toBeTruthy();
    ...
    expect(screen.getByText('Add places to your itinerary to see them on the map.')).toBeTruthy();
  ...
  it('navigates to PlaceDetail when preview card body is pressed', async () => {
    ...
    const previewCard = screen.getByLabelText(/Breakfast at Ro Roast, 09:00 AM/);
  ...
  it('preserves and renders newly added place from P11 AddPlace flow', async () => {
    ...
    expect(screen.getByLabelText(/Location 5: Jim Thompson House/)).toBeTruthy();
  });
  ...
  it('allows navigation to TripMap from TripDetail top bar and bento card', async () => {
    ...
    await renderWithProviders(<TripDetailScreen navigation={mockNavigation} route={route} />);
