...
function createRepositories(
  restore: Promise<AuthenticatedSession | null> = Promise.resolve(null),
) {
  ...
  return {
    ...
    profiles,
    unsubscribe,
    emit: (value: AuthenticatedSession | null) => listener(value),
  };
}
  ...
  return (
    <View>
      ...
      <Button
        ...
        onPress={() =>
          void auth
            .signIn("owner@example.com", "not-logged")
            .catch(() => setFailure("sign-in"))
        }
      />
      <Button
        title="sign-out"
        onPress={() => void auth.signOut().catch(() => setFailure("sign-out"))}
      />
      <Button
        title="update"
        onPress={() => void auth.updateProfile({ displayName: "Updated" })}
      />
    </View>
  );
}
...
describe("AuthProvider session lifecycle", () => {
  ...
  it("keeps signed-in state when remote sign-out fails and reconciles a successful profile update", async () => {
    ...
    await waitFor(() =>
      expect(repositories.profiles.updateOwnProfile).toHaveBeenCalledWith(
        userId,
        { displayName: "Updated" },
      ),
    );
  ...
  it("unsubscribes and ignores pending profile work after unmount", async () => {
    ...
    await waitFor(() =>
      expect(screen.getByTestId("profile-status").props.children).toBe(
        "loading",
      ),
    );
    await view.unmount();
