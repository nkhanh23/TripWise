...
import {
  resetSavedPlaces,
  savePlace,
} from "../src/features/saved/data/savedPlacesStore";
...
describe("Responsive & Android Performance Audit (FE-P17-T001)", () => {
  ...
  describe("2. Large Collection Virtualization & Stable Keys Stress Audit", () => {
    ...
    it("virtualizes 50 Route steps with stable keyExtractor", async () => {
      ...
      expect(
        screen.getByText("Turn left onto Rama I Rd (Step #1)"),
      ).toBeTruthy();
      expect(
        screen.getByText("Turn right onto Phloen Chit Rd (Step #2)"),
      ).toBeTruthy();
    });
    ...
    it("virtualizes large Saved Places list with memoized renderItem and stable keys", async () => {
      ...
      await renderWithProviders(<SavedPlacesScreen customPlaces={saved40} />);
