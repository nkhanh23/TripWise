...
} from "../src/integration/reliability";
...
describe("integration idempotency and reliability", () => {
  it("preserves a key for one intent and creates a new key for another intent", () => {
    ...
    const factory = new IdempotencyKeyFactory(
      () => keys.shift() ?? "intent-key-fallback",
    );
    ...
    first.complete();
    expect(() => first.key()).toThrow(
      "Invalid completed save intent contract.",
    );
  });
  ...
  it("retries transient failures within a bounded attempt count", async () => {
    ...
    await expect(
      executeWithReliability(operation, {
        timeoutMs: 100,
        maximumAttempts: 2,
        retryDelayMs: 0,
      }),
  ...
  it.each(["invalidRequest", "unauthorized", "conflict"] as const)(
    "does not retry %s",
    async (code) => {
  ...
  it("maps timeout and supports external cancellation", async () => {
    ...
    await expect(
      executeWithReliability(never, { timeoutMs: 5, maximumAttempts: 1 }),
    ).rejects.toMatchObject({ code: "timeout" });

    const controller = new AbortController();
    const pending = executeWithReliability(
      never,
      { timeoutMs: 100, maximumAttempts: 1 },
      controller.signal,
    );
    controller.abort();
  ...
  it("keeps one bounded client attempt with margin above the Edge provider timeout", () => {
    expect(tripGenerationPolicy).toEqual({
      timeoutMs: 50_000,
      maximumAttempts: 1,
    });
  });
