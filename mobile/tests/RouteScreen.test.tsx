...
describe("RoutePreviewScreen", () => {
  ...
  it("renders route preview with map canvas, origin/destination, transport chips, summary card, and steps", async () => {
    ...
    expect(
      screen.getByText("Fastest route via BTS Sukhumvit Line"),
    ).toBeTruthy();
    ...
    expect(
      screen.getByText("BTS Sukhumvit Line (Towards Khu Khot)"),
    ).toBeTruthy();
    expect(screen.getByText("Transfer at Siam Station")).toBeTruthy();
    expect(screen.getByText("Walk to Destination")).toBeTruthy();

    // Bottom CTA
    const ctaButton = screen.getByText("Start Route");
    expect(ctaButton).toBeTruthy();
    // Since we disabled the button, it shouldn't trigger an alert
    const pressable = screen.getByLabelText("Start Route");
