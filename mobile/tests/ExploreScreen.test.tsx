...
import {
  largeMockExplorePlaces,
  mockExplorePlaces,
} from "../src/features/explore/data/mockPlaces";
...
describe("ExploreScreen", () => {
  ...
  it("renders explore screen with map canvas, search bar, category chips, and markers", async () => {
    ...
    // Search bar
    expect(
      screen.getByPlaceholderText("Search Tokyo, Bangkok..."),
    ).toBeTruthy();
    ...
    // Map canvas
    expect(
      screen.getByLabelText(/Interactive Map|Bản đồ tương tác/),
    ).toBeTruthy();
  ...
  it("toggles between Map and List view modes", async () => {
    ...
    expect(
      screen.queryByLabelText(/Interactive Map|Bản đồ tương tác/),
    ).toBeNull();
  ...
  it("synchronizes selection between List and Map modes", async () => {
    ...
    await user.press(
      screen.getByLabelText("Wat Arun, Buddhist Temple, đánh giá 4.8 sao"),
    );
  ...
  it("renders cluster markers for dense places on map and allows selection", async () => {
    ...
    const clusterMarkers = screen.queryAllByLabelText(
      /địa điểm trong khu vực này/,
    );
  ...
  it("renders error state and recovers on retry", async () => {
    const user = userEvent.setup();
    await render(
      <ExploreScreen initialPlaces={mockExplorePlaces} initialStatus="error" />,
    );
