...
describe("MyTripsScreen", () => {
  ...
  it("renders MyTripsScreen with top bar, headings, upcoming/past sections, and FAB", async () => {
    ...
    expect(
      screen.getByText(
        "Manage your upcoming journeys and review past adventures.",
      ),
    ).toBeTruthy();
  ...
  it("handles trip card selection and navigates with tripId", async () => {
    ...
    await user.press(
      screen.getByLabelText(
        "Kyoto Autumn Retreat, Kyoto, Japan, Oct 14 - Oct 22",
      ),
    );
  ...
  it("enriches a remote trip card from the trusted compact cover identity without detail N+1", async () => {
    ...
    const getPhoto = jest.fn(
      async ({ googlePlaceId }: { googlePlaceId: string }) => ({
        googlePlaceId,
    ...
    expect(
      await screen.findByLabelText("Bangkok Explorer cover photo"),
    ).toBeTruthy();
  ...
  it("preserves the second-candidate Wikimedia image and attribution through hook state and card rerender", async () => {
    ...
    const destination: DestinationCoverRepository = {
      getDestinationCover: jest.fn(async () => ({
        uri: null,
    ...
    await render(
      <MyTripsScreen
        repository={repository}
        tripCoverRepository={
          new SequentialTripCoverImageRepository(google, wikipedia, destination)
        }
      />,
    );
  ...
  it("does not cancel and restart the same cover work when focus reload returns identical identities", async () => {
    ...
    await render(
      <MyTripsScreen
        repository={repository}
        tripCoverRepository={{ getTripCover }}
      />,
    );
    expect(
      await screen.findByLabelText("Bangkok Explorer cover photo"),
  ...
  it("deduplicates a focus refresh while the initial trip-list request is still pending", async () => {
    let resolveList:
      | ((value: { items: SavedTripSummary[]; nextCursor: null }) => void)
      | undefined;
    const list = jest.fn(
      () =>
        new Promise<{ items: SavedTripSummary[]; nextCursor: null }>(
          (resolve) => {
            resolveList = resolve;
          },
        ),
