...
exports.validateExplorePlacesRequest = validateExplorePlacesRequest;
exports.parseExplorePlacesSuccess = parseExplorePlacesSuccess;
exports.isUuid = isUuid;
exports.isIsoDate = isIsoDate;
exports.isIsoTimestamp = isIsoTimestamp;
exports.inclusiveDurationDays = inclusiveDurationDays;
exports.asUserId = asUserId;
exports.asTripId = asTripId;
exports.asItineraryItemId = asItineraryItemId;
exports.validateWorkspaceMutationCommand = validateWorkspaceMutationCommand;
exports.parseWorkspaceMutationResult = parseWorkspaceMutationResult;
exports.asSavedPlaceId = asSavedPlaceId;
exports.asGooglePlaceId = asGooglePlaceId;
exports.parseProfileTransport = parseProfileTransport;
exports.parseProfileStatistics = parseProfileStatistics;
exports.validateGenerateTripRequest = validateGenerateTripRequest;
exports.parseGenerateTripSuccess = parseGenerateTripSuccess;
exports.validatePersistTripCommand = validatePersistTripCommand;
exports.validateSavedTripsPageRequest = validateSavedTripsPageRequest;
exports.parseSavedTripsPage = parseSavedTripsPage;
exports.parseSavedTripDetail = parseSavedTripDetail;
exports.validateResolvePlaceRequest = validateResolvePlaceRequest;
exports.parseResolvePlaceSuccess = parseResolvePlaceSuccess;
exports.isValidCoordinate = isValidCoordinate;
exports.validateRouteRequest = validateRouteRequest;
exports.parseOsrmRoute = parseOsrmRoute;
exports.validateRouteTableRequest = validateRouteTableRequest;
exports.parseOsrmTable = parseOsrmTable;
exports.validateWeatherRequest = validateWeatherRequest;
exports.parseOpenMeteoForecast = parseOpenMeteoForecast;
exports.validateGetPlacePhotoRequest = validateGetPlacePhotoRequest;
exports.parseGetPlacePhotoSuccess = parseGetPlacePhotoSuccess;
exports.validateWikimediaImageRequest = validateWikimediaImageRequest;
exports.parseWikimediaImageSuccess = parseWikimediaImageSuccess;
exports.validateSavePlaceCommand = validateSavePlaceCommand;
exports.parseSavedPlaceTransport = parseSavedPlaceTransport;
exports.parseSavedPlacesPage = parseSavedPlacesPage;
exports.validateCreateTripExpenseCommand = validateCreateTripExpenseCommand;
exports.validateUpdateTripExpenseCommand = validateUpdateTripExpenseCommand;
exports.validateDeleteTripExpenseCommand = validateDeleteTripExpenseCommand;
exports.validateListTripExpensesRequest = validateListTripExpensesRequest;
exports.parseTripExpenseRecord = parseTripExpenseRecord;
exports.parseTripExpensesPage = parseTripExpensesPage;
...
function validateExplorePlacesRequest(value) {
    if (!isRecord(value) || !hasOnlyKeys(value, ['center', 'radiusMeters', 'category', 'limit'])
        ...
        || !Number.isFinite(value.radiusMeters) || value.radiusMeters < 100
        || value.radiusMeters > 5_000
        || typeof value.category !== 'string'
        || !exploreCategories.includes(value.category)
        || (value.limit !== undefined
            && (!Number.isInteger(value.limit) || value.limit < 1 || value.limit > 12))) {
        throw new ContractValidationError('explore-places request');
    }
    return value;
}
function parseExplorePlacesSuccess(value) {
    ...
    const places = value.data.places.map((place) => {
        if (!isRecord(place)
            ...
            || !exploreCategories.includes(place.category)
        ...
        return {
            ...
            ...(place.formattedAddress === undefined ? {} : { address: place.formattedAddress.trim() }),
...
function nullableString(value, maximumLength) {
...
function asUserId(value) {
    if (!isUuid(value))
        throw new ContractValidationError('user ID');
    return value;
}
function asTripId(value) {
    if (!isUuid(value))
        throw new ContractValidationError('trip ID');
    return value;
}
function asItineraryItemId(value) {
    if (!isUuid(value))
        throw new ContractValidationError('itinerary item ID');
    return value;
}
const workspaceFlexibilities = ['fixed', 'flexible'];
const workspacePriorities = ['must_do', 'want_to_do', 'optional'];
const workspaceStatuses = ['scheduled', 'completed', 'skipped'];
const workspaceKinds = ['place', 'custom_activity', 'restaurant', 'transport', 'accommodation', 'reservation', 'note'];
const sourceLinkTypes = ['google_maps', 'facebook', 'instagram', 'tiktok', 'website', 'booking', 'other'];
const transportModes = ['walk', 'drive', 'transit', 'bus', 'train', 'flight', 'motorbike', 'ferry', 'other'];
function invalidNullableBoundedString(value, maximum) {
    if (value === undefined || value === null)
        return false;
...
function validateWorkspaceItemPatch(value) {
    const allowed = ['placeName', 'placeQuery', 'flexibility', 'priority', 'startTime', 'endTime', 'note', 'contact', 'transport', 'accommodation'];
    if (!isRecord(value) || !hasOnlyKeys(value, allowed) || Object.keys(value).length === 0)
        throw new ContractValidationError('workspace item patch');
    if (value.placeName !== undefined && requiredString(value.placeName, 160) === null)
        throw new ContractValidationError('workspace item patch');
    if (invalidNullableBoundedString(value.placeQuery, 200))
        throw new ContractValidationError('workspace item patch');
    if (value.flexibility !== undefined && (typeof value.flexibility !== 'string' || !workspaceFlexibilities.includes(value.flexibility)))
        throw new ContractValidationError('workspace item patch');
    if (value.priority !== undefined && (typeof value.priority !== 'string' || !workspacePriorities.includes(value.priority)))
        throw new ContractValidationError('workspace item patch');
    for (const key of ['startTime', 'endTime'])
        if (value[key] !== undefined && value[key] !== null && (typeof value[key] !== 'string' || !timePattern.test(value[key])))
            throw new ContractValidationError('workspace item patch');
    if (invalidNullableBoundedString(value.note, 500))
        throw new ContractValidationError('workspace item patch');
    if (value.contact !== undefined) {
        if (!isRecord(value.contact))
            throw new ContractValidationError('workspace contact patch');
        ...
        if (!hasOnlyKeys(contact, ['name', 'phone', 'address', 'websiteUrl', 'bookingUrl', 'reservationCode'])
            ...
            || (contact.bookingUrl !== undefined && contact.bookingUrl !== null && !validHttpsUrl(contact.bookingUrl)))
            throw new ContractValidationError('workspace contact patch');
    }
    if (value.transport !== undefined) {
        if (!isRecord(value.transport))
            throw new ContractValidationError('workspace transport patch');
        ...
        if (!hasOnlyKeys(transport, ['mode', 'originLabel', 'destinationLabel', 'operatorName', 'departureAt', 'arrivalAt', 'plannedCostAmount', 'plannedCostCurrency'])
            || (transport.mode !== undefined && transport.mode !== null && (typeof transport.mode !== 'string' || !transportModes.includes(transport.mode)))
            ...
            || ('plannedCostAmount' in transport && 'plannedCostCurrency' in transport && ((typeof transport.plannedCostAmount === 'number') !== (typeof transport.plannedCostCurrency === 'string'))))
            throw new ContractValidationError('workspace transport patch');
    }
    if (value.accommodation !== undefined) {
        if (!isRecord(value.accommodation))
            throw new ContractValidationError('workspace accommodation patch');
        ...
        if (!hasOnlyKeys(accommodation, ['checkInAt', 'checkOutAt', 'nights'])
            ...
            || (typeof accommodation.nights === 'number' && 'checkInAt' in accommodation && 'checkOutAt' in accommodation
                && (typeof accommodation.checkInAt !== 'string' || typeof accommodation.checkOutAt !== 'string'
                    || Math.round((utcDateMs(accommodation.checkOutAt) - utcDateMs(accommodation.checkInAt)) / 86_400_000) !== accommodation.nights)))
            throw new ContractValidationError('workspace accommodation patch');
    }
...
function validateCreateCustomActivity(value) {
    if (!isRecord(value) || !hasOnlyKeys(value, ['itemKind', 'title', 'flexibility', 'priority', 'startTime', 'endTime'])
        ...
        || typeof value.flexibility !== 'string' || !workspaceFlexibilities.includes(value.flexibility)
        || typeof value.priority !== 'string' || !workspacePriorities.includes(value.priority)
    ...
    return {
        itemKind: 'custom_activity', title: value.title.trim(),
...
function validateWorkspaceLinks(value) {
    if (!Array.isArray(value) || value.length > 12)
        throw new ContractValidationError('workspace source links');
    return value.map((link) => {
        if (!isRecord(link) || !hasOnlyKeys(link, ['type', 'url', 'label']) || typeof link.type !== 'string'
            || !sourceLinkTypes.includes(link.type) || !validHttpsUrl(link.url)
            || (link.label !== undefined && requiredString(link.label, 120) === null)
            || (link.type === 'other' && requiredString(link.label, 120) === null))
            throw new ContractValidationError('workspace source link');
        return { type: link.type, url: link.url.trim(), ...(link.label === undefined ? {} : { label: link.label.trim() }) };
    });
}
function validateWorkspaceMutationCommand(value) {
    if (!isRecord(value) || !hasOnlyKeys(value, ['type', 'tripId', 'dayId', 'itemId', 'expectedRevision', 'item', 'patch', 'status', 'links', 'targetDayId', 'targetPosition'])
        || !isUuid(value.tripId) || !Number.isInteger(value.expectedRevision) || typeof value.expectedRevision !== 'number' || value.expectedRevision < 1 || typeof value.type !== 'string')
    ...
    if (value.type === 'create_item' && isUuid(value.dayId) && value.itemId === undefined && value.patch === undefined && value.status === undefined && value.links === undefined) {
        return { type: 'create_item', tripId: value.tripId, dayId: value.dayId, expectedRevision: value.expectedRevision, item: validateCreateCustomActivity(value.item) };
    }
    if (!isUuid(value.itemId) || value.dayId !== undefined || value.item !== undefined)
        throw new ContractValidationError('workspace mutation command');
    const base = { tripId: value.tripId, itemId: value.itemId, expectedRevision: value.expectedRevision };
    if (value.type === 'move_item' && value.patch === undefined && value.status === undefined && value.links === undefined
        ...
        return { type: 'move_item', ...base, targetDayId: value.targetDayId, targetPosition: value.targetPosition };
    }
    if (value.targetDayId !== undefined || value.targetPosition !== undefined)
        throw new ContractValidationError('workspace mutation command');
    if (value.type === 'update_item' && value.status === undefined && value.links === undefined)
        return { type: 'update_item', ...base, patch: validateWorkspaceItemPatch(value.patch) };
    if (value.type === 'transition_item_status' && value.patch === undefined && value.links === undefined && typeof value.status === 'string' && workspaceStatuses.includes(value.status))
        return { type: 'transition_item_status', ...base, status: value.status };
    if (value.type === 'replace_source_links' && value.patch === undefined && value.status === undefined)
        return { type: 'replace_source_links', ...base, links: validateWorkspaceLinks(value.links) };
    throw new ContractValidationError('workspace mutation command');
}
function parseWorkspaceMutationResult(value) {
    if (!isRecord(value) || !hasOnlyKeys(value, ['revision', 'itemId', 'noOp']) || !Number.isInteger(value.revision) || typeof value.revision !== 'number' || value.revision < 1 || (value.itemId !== undefined && !isUuid(value.itemId)) || (value.noOp !== undefined && typeof value.noOp !== 'boolean'))
        throw new ContractValidationError('workspace mutation result');
    return { revision: value.revision, ...(value.itemId === undefined ? {} : { itemId: value.itemId }), ...(value.noOp === undefined ? {} : { noOp: value.noOp }) };
}
function asSavedPlaceId(value) {
    if (!isUuid(value))
        throw new ContractValidationError('saved place ID');
    return value;
}
function asGooglePlaceId(value) {
    if (typeof value !== 'string' || !/^[A-Za-z0-9_-]{10,200}$/.test(value)) {
        throw new ContractValidationError('google place ID');
    }
    return value;
}
function parseProfileTransport(value) {
    if (!isRecord(value)
...
function parseProfileStatistics(value) {
    if (!isRecord(value)
...
function validateGenerateTripRequest(value) {
    if (!isRecord(value)
    ...
    if (value.travelers !== undefined
        && (!Number.isInteger(value.travelers) || value.travelers < 1 || value.travelers > 20)) {
        throw new ContractValidationError('generate-trip request');
    }
    ...
    return {
        ...
        ...(value.travelers === undefined ? {} : { travelers: value.travelers }),
...
function parseGeneratedItem(value, expectedPosition) {
    if (!isRecord(value)
...
function parseGeneratedDay(value, expectedDay, expectedDate) {
    if (!isRecord(value)
...
function parseGenerateTripSuccess(value) {
    if (!isRecord(value) || !hasOnlyKeys(value, ['data']) || !isRecord(value.data)) {
    ...
    const days = trip.days.map((day, index) => parseGeneratedDay(day, index + 1, new Date(start + index * dayMilliseconds).toISOString().slice(0, 10)));
    return {
        data: {
            title,
            destination,
...
function validatePersistenceGraph(graph) {
    if (!isRecord(graph)
    ...
    return {
        ...
        ...(graph.estimatedBudget === undefined ? {} : { estimatedBudget: graph.estimatedBudget }),
        ...(graph.currency === undefined ? {} : { currency: graph.currency }),
        days,
    };
}
function validatePersistTripCommand(value) {
...
function validateSavedTripsPageRequest(value) {
    ...
    return { limit: limit, cursor: value.cursor === undefined ? null : parseCursor(value.cursor) };
}
function parseSavedTripsPage(value) {
    ...
    const items = value.items.map((item) => {
        ...
        const validCoverGooglePlaceIds = Array.isArray(coverGooglePlaceIds)
            ...
            && coverGooglePlaceIds.every((placeId) => typeof placeId === 'string' && /^[A-Za-z0-9_-]{10,200}$/.test(placeId));
        ...
        return {
            ...
            dayCount,
            itemCount,
            coverGooglePlaceIds: coverGooglePlaceIds.map((placeId) => placeId),
        };
    });
    return { items, nextCursor: parseCursor(value.nextCursor) };
}
function parseSavedTripItem(value, expectedPosition) {
    if (!isRecord(value)
        || !isUuid(value.id)
        || value.position !== expectedPosition
    ...
    try {
        if (value.contact !== undefined)
            metadata.contact = validateWorkspaceItemPatch({ contact: value.contact }).contact;
        if (value.transport !== undefined)
            metadata.transport = validateWorkspaceItemPatch({ transport: value.transport }).transport;
        if (value.accommodation !== undefined)
            metadata.accommodation = validateWorkspaceItemPatch({ accommodation: value.accommodation }).accommodation;
    }
    catch {
    ...
    if (!placeName || placeQuery === null || note === null
        || typeof itemKind !== 'string' || !workspaceKinds.includes(itemKind)
        || typeof flexibility !== 'string' || !workspaceFlexibilities.includes(flexibility)
        || typeof priority !== 'string' || !workspacePriorities.includes(priority)
        || typeof activityStatus !== 'string' || !workspaceStatuses.includes(activityStatus)
...
function parseSavedTripDay(value, expectedDay, startDate) {
...
function parseSavedTripDetail(value) {
    if (value === null)
        return null;
    if (!isRecord(value)
    ...
    return {
        ...(Object.hasOwn(value, 'timezone') ? { timezone: (0, tripTimezone_1.parseTripTimezone)(value.timezone) } : {}),
        id: value.id,
        title,
        destination,
        startDate: value.startDate,
        ...
        days: value.days.map((day, index) => parseSavedTripDay(day, index + 1, value.startDate)),
    };
}
function validateResolvePlaceRequest(value) {
...
function parseResolvePlaceSuccess(value) {
    if (!isRecord(value) || !hasOnlyKeys(value, ['data']) || !isRecord(value.data)
    ...
    return {
        data: {
            itineraryItemId: value.data.itineraryItemId,
...
function parseOsrmRoute(value) {
    ...
    return {
        ...
        routes: [{
                ...
                geometry: {
                    type: 'LineString',
                    coordinates: route.geometry.coordinates.map((point) => [point[0], point[1]]),
                },
            }],
    };
}
function validateRouteTableRequest(value) {
    if (!isRecord(value) || !hasOnlyKeys(value, ['profile', 'coordinates']) || value.profile !== 'driving'
        || !Array.isArray(value.coordinates) || value.coordinates.length < 2 || value.coordinates.length > 25
        || !value.coordinates.every(isValidCoordinate)) {
...
function parseOsrmTable(value, expectedCoordinatesCount, options) {
    ...
    const durations = value.durations.map((row) => {
        if (!Array.isArray(row))
            throw new ContractValidationError('OSRM table durations row');
        if (row.length !== expectedCols)
            throw new ContractValidationError('OSRM table duration column count');
        return row.map((d) => {
            if (d === null)
                return null;
            const parsed = finiteNumber(d, 0, Number.MAX_SAFE_INTEGER);
            if (parsed === null)
                throw new ContractValidationError('OSRM table duration value');
            return parsed;
        });
    });
    let distances;
    if (value.distances !== undefined) {
        if (!Array.isArray(value.distances))
            throw new ContractValidationError('OSRM table distances');
        ...
        distances = value.distances.map((row) => {
            if (!Array.isArray(row))
                throw new ContractValidationError('OSRM table distances row');
            if (row.length !== expectedCols)
                throw new ContractValidationError('OSRM table distance column count');
            return row.map((d) => {
                if (d === null)
                    return null;
                const parsed = finiteNumber(d, 0, Number.MAX_SAFE_INTEGER);
                if (parsed === null)
                    throw new ContractValidationError('OSRM table distance value');
                return parsed;
            });
        });
    }
...
function validateWeatherRequest(value) {
    if (!isRecord(value) || !hasOnlyKeys(value, ['latitude', 'longitude', 'forecastDays'])
        ...
        || !Number.isInteger(value.forecastDays) || value.forecastDays < 1 || value.forecastDays > 16) {
...
function parseNullableNumberArray(value, length, minimum, maximum) {
...
function parseOpenMeteoForecast(value) {
...
function validateGetPlacePhotoRequest(value) {
...
function parseGetPlacePhotoSuccess(value) {
    if (!isRecord(value) || !hasOnlyKeys(value, ['data']) || !isRecord(value.data)
    ...
    let authorAttribution = undefined;
...
function validateWikimediaImageRequest(value) {
...
function parseWikimediaImageSuccess(value) {
    if (!isRecord(value) || !hasOnlyKeys(value, ['data']) || !isRecord(value.data)
    ...
    if (value.data.attribution !== undefined) {
        ...
        attribution = {
            displayName: value.data.attribution.displayName,
            sourceUrl: value.data.attribution.sourceUrl,
            ...(value.data.attribution.license === undefined ? {} : { license: value.data.attribution.license }),
            ...(value.data.attribution.licenseUrl === undefined ? {} : { licenseUrl: value.data.attribution.licenseUrl }),
        };
    }
    ...
    return {
        data: {
            ...
            ...(value.data.matchedEntity === undefined ? {} : { matchedEntity: value.data.matchedEntity }),
            ...(typeof confidence === 'number' ? { confidence } : {}),
        },
    };
}
function validateSavePlaceCommand(value) {
...
function parseSavedPlaceTransport(value) {
...
function parseSavedPlacesPage(value) {
    if (!isRecord(value) || !hasOnlyKeys(value, ['items', 'nextCursor']) || !Array.isArray(value.items)) {
...
const expenseCategories = ['food', 'transport', 'accommodation', 'activity', 'shopping', 'ticket', 'personal', 'reservation', 'other'];
const expenseOrigins = ['planned', 'actual', 'unplanned'];
function validateCreateTripExpenseCommand(command) {
    if (!isRecord(command))
    ...
    if (!tripId || !isUuid(tripId))
        throw new ContractValidationError('create trip expense command: tripId');
    if (typeof command.category !== 'string' || !expenseCategories.includes(command.category)) {
        throw new ContractValidationError('create trip expense command: category');
    }
    const category = command.category;
    if (typeof command.origin !== 'string' || !expenseOrigins.includes(command.origin)) {
    ...
    if (command.note !== undefined && command.note !== null) {
        if (typeof command.note !== 'string')
            throw new ContractValidationError('create trip expense command: note');
    ...
    if (command.itineraryItemId !== undefined && command.itineraryItemId !== null) {
        ...
        itineraryItemId = command.itineraryItemId;
    }
    return {
        tripId: tripId,
        category,
...
function validateUpdateTripExpenseCommand(command) {
    if (!isRecord(command))
    ...
    if (!expenseId || !isUuid(expenseId))
        throw new ContractValidationError('update trip expense command: expenseId');
    const tripId = requiredString(command.tripId, 64);
    if (!tripId || !isUuid(tripId))
        throw new ContractValidationError('update trip expense command: tripId');
    ...
    if (patch.category !== undefined) {
        if (typeof patch.category !== 'string' || !expenseCategories.includes(patch.category)) {
            throw new ContractValidationError('update trip expense command: patch.category');
        }
        validatedPatch.category = patch.category;
    }
    if (patch.origin !== undefined) {
        if (typeof patch.origin !== 'string' || !expenseOrigins.includes(patch.origin)) {
            throw new ContractValidationError('update trip expense command: patch.origin');
        }
        validatedPatch.origin = patch.origin;
    }
    ...
    if (patch.itineraryItemId !== undefined) {
        if (patch.itineraryItemId === null) {
            ...
            validatedPatch.itineraryItemId = patch.itineraryItemId;
        }
        else {
...
function validateDeleteTripExpenseCommand(command) {
...
function validateListTripExpensesRequest(request) {
    ...
    if (!tripId || !isUuid(tripId))
        throw new ContractValidationError('list trip expenses request: tripId');
    let limit = 20;
    ...
    if (request.cursor !== undefined && request.cursor !== null) {
        ...
        cursor = { createdAt, id: id };
    }
    let category;
    if (request.category !== undefined) {
        if (typeof request.category !== 'string' || !expenseCategories.includes(request.category)) {
    ...
    if (request.origin !== undefined) {
        if (typeof request.origin !== 'string' || !expenseOrigins.includes(request.origin)) {
...
function parseTripExpenseRecord(value) {
    ...
    if (typeof value.category !== 'string' || !expenseCategories.includes(value.category)) {
        throw new ContractValidationError('trip expense record: category');
    }
    const category = value.category;
    if (typeof value.origin !== 'string' || !expenseOrigins.includes(value.origin)) {
    ...
    if (value.note !== null && value.note !== undefined) {
        if (typeof value.note !== 'string')
            throw new ContractValidationError('trip expense record: note');
...
function parseTripExpensesPage(value) {
    if (!isRecord(value) || !hasOnlyKeys(value, ['items', 'nextCursor']) || !Array.isArray(value.items)) {
