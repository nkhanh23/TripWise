...
exports.PLANNING_BOUNDS = void 0;
exports.isValidTimeString = isValidTimeString;
exports.timeToMinutes = timeToMinutes;
exports.isFixedItem = isFixedItem;
exports.isMustDoItem = isMustDoItem;
exports.isValidFlexibility = isValidFlexibility;
exports.isValidPriority = isValidPriority;
exports.sortConflictsDeterministically = sortConflictsDeterministically;
exports.normalizeItineraryInput = normalizeItineraryInput;
exports.evaluatePlanConstraints = evaluatePlanConstraints;
exports.evaluatePlanModification = evaluatePlanModification;
// ============================================================================
// Planning Bounds
// ============================================================================
exports.PLANNING_BOUNDS = {
    MAX_DAYS: 60,
 ...
 */
function parseStrictTimeScalar(rawVal, fieldName, itemId, dayNumber, origin, placeName) {
 ...
 */
function sortConflictsDeterministically(conflicts, itemStartTimeMap) {
 ...
 */
function normalizeItineraryInput(input, origin = 'proposed') {
    ...
    else if ('days' in input && Array.isArray(input.days)) {
        rawDays = input.days;
    }
    else {
        malformedConflicts.push({
    ...
    if (rawDays.length > exports.PLANNING_BOUNDS.MAX_DAYS) {
        malformedConflicts.push({
            code: 'MALFORMED_INPUT',
            itemId: 'itinerary',
            origin,
            message: `Itinerary exceeds maximum allowed planning days (${rawDays.length} > ${exports.PLANNING_BOUNDS.MAX_DAYS})`,
            details: { maxDays: exports.PLANNING_BOUNDS.MAX_DAYS, actualDays: rawDays.length },
        });
        return { days: [], malformedConflicts };
    }
    const dayMap = new Map();
    const seenDayNumbers = new Set();
    const dayNumbersInOrder = [];
    ...
    for (let dIdx = 0; dIdx < rawDays.length; dIdx++) {
        ...
        if (rawItems.length > exports.PLANNING_BOUNDS.MAX_ITEMS_PER_DAY) {
            malformedConflicts.push({
                code: 'MALFORMED_INPUT',
                itemId: `day-${dayNumber}`,
                dayNumber,
                origin,
                message: `Day ${dayNumber} exceeds maximum allowed items per day (${rawItems.length} > ${exports.PLANNING_BOUNDS.MAX_ITEMS_PER_DAY})`,
                details: { maxItemsPerDay: exports.PLANNING_BOUNDS.MAX_ITEMS_PER_DAY, actualItems: rawItems.length },
            });
            continue;
        }
        ...
        if (totalItemsCount + rawItems.length > exports.PLANNING_BOUNDS.MAX_TOTAL_ITEMS) {
            malformedConflicts.push({
                code: 'MALFORMED_INPUT',
                itemId: 'itinerary',
                origin,
                message: `Itinerary exceeds maximum allowed total items (${totalItemsCount + rawItems.length} > ${exports.PLANNING_BOUNDS.MAX_TOTAL_ITEMS})`,
                details: {
                    maxTotalItems: exports.PLANNING_BOUNDS.MAX_TOTAL_ITEMS,
        ...
        const items = [];
        const seenPositionsOnDay = new Set();
        const positionsOnDay = [];
 ...
 */
function findFixedOverlapsOnDay(fixedItemsOnDay, dayNumber, origin) {
    const overlaps = [];
 ...
 */
function evaluateIntraSchedule(normalized, itemStartTimeMap, origin = 'proposed') {
    const conflicts = [...normalized.malformedConflicts];
    const flattenedItems = new Map();
    ...
    for (const [id, occurrences] of idOccurrences.entries()) {
        ...
        if (isFixedItem(primary.item))
            fixedCount++;
        if (isMustDoItem(primary.item))
            mustDoCount++;
        ...
        for (const item of day.items) {
            ...
            if (startMin !== null && endMin !== null) {
                if (endMin < startMin) {
                    ...
                    const code = isFixed ? 'FIXED_INVALID_TIME_RANGE' : 'INVALID_TIME_RANGE';
                    conflicts.push({
                        code,
                        itemId: item.id,
 ...
 */
function evaluatePlanDelta(baselineItems, proposedItems) {
    const deltaConflicts = [];
 ...
 */
function evaluatePlanConstraints(proposed, baseline) {
    const itemStartTimeMap = new Map();
    ...
    const allConflicts = [...proposedAnalysis.conflicts];
    ...
    if (baseline !== undefined) {
        ...
        const deltaConflicts = evaluatePlanDelta(baselineAnalysis.flattenedItems, proposedAnalysis.flattenedItems);
 ...
 */
function evaluatePlanModification(baseline, proposed) {
