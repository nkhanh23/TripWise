...
export function parseProfileTransport(value: unknown): ProfileTransport {
  if (
    ...
    (nullableString(value.display_name, 160) === null &&
      value.display_name !== null) ||
    (nullableString(value.avatar_url, 2_048) === null &&
...
export function validateGenerateTripRequest(
  ...
  if (duration < 1 || duration > 14)
    throw new ContractValidationError("generate-trip request");
  if (
    value.travelers !== undefined &&
    (!Number.isInteger(value.travelers) ||
      (value.travelers as number) < 1 ||
      (value.travelers as number) > 20)
  ) {
    throw new ContractValidationError("generate-trip request");
  }
  if (
    value.budget !== undefined &&
    finiteNumber(value.budget, 0, 1_000_000_000) === null
  ) {
    throw new ContractValidationError("generate-trip request");
  }
  if (
...
function parseGeneratedItem(
  ...
  if (
    ...
    finiteNumber(value.estimatedCost, 0, 1_000_000_000) === null
  ) {
    throw new ContractValidationError("generated itinerary item");
  }
  return {
...
function parseGeneratedDay(
  ...
  return {
    ...
    items: value.items.map((item, index) =>
      parseGeneratedItem(item, index + 1),
    ),
  };
}

export function parseGenerateTripSuccess(
  value: unknown,
): GenerateTripSuccessEnvelope {
  if (
    !isRecord(value) ||
...
function validatePersistenceGraph(graph: unknown): TripGraphPayload {
  ...
  if (
    !title ||
    !destination ||
    !isIsoDate(graph.startDate) ||
    !isIsoDate(graph.endDate) ||
  ...
  if (
    graph.estimatedBudget !== undefined &&
    graph.estimatedBudget !== null &&
    finiteNumber(graph.estimatedBudget, 0, 1_000_000_000) === null
  ) {
    throw new ContractValidationError("trip graph");
  }
  if (
    graph.currency !== undefined &&
    graph.currency !== null &&
    (typeof graph.currency !== "string" ||
      !currencyPattern.test(graph.currency))
  ) {
    throw new ContractValidationError("trip graph");
  }
  let totalItems = 0;
  ...
  const days = graph.days.map((rawDay, dayIndex) => {
    if (
      ...
      rawDay.date !==
        new Date(start + dayIndex * dayMilliseconds)
          .toISOString()
          .slice(0, 10) ||
    ...
    return {
      ...
      items: rawDay.items.map((rawItem, itemIndex) => {
        if (
          ...
          Object.hasOwn(rawItem, "latitude") !==
            Object.hasOwn(rawItem, "longitude") ||
        ...
        const item = parseGeneratedItem(
          {
            ...
            ...(rawItem.placeQuery === undefined
              ? {}
              : { placeQuery: rawItem.placeQuery }),
            ...(rawItem.startTime === undefined
              ? {}
              : { startTime: rawItem.startTime }),
            ...(rawItem.endTime === undefined
              ? {}
  ...
  return {
    ...
    ...(graph.estimatedBudget === undefined
      ? {}
      : { estimatedBudget: graph.estimatedBudget as number | null }),
    ...(graph.currency === undefined
      ? {}
      : { currency: graph.currency as string | null }),
    days,
  };
}
...
export function validatePersistTripCommand(value: unknown): PersistTripCommand {
  if (
    !isRecord(value) ||
    !hasOnlyKeys(value, ["idempotencyKey", "graph"]) ||
  ...
  return {
    ...
    graph: validatePersistenceGraph(value.graph),
  };
}
...
function parseCursor(value: unknown): SavedTripCursor | null {
  ...
  if (
    !isRecord(value) ||
    !hasOnlyKeys(value, ["createdAt", "id"]) ||
...
export function validateSavedTripsPageRequest(value: unknown): {
  limit: number;
  ...
  const items = value.items.map((item) => {
    ...
    const estimatedBudget =
      item.estimatedBudget === null
        ? null
        : finiteNumber(item.estimatedBudget, 0, 1_000_000_000);
    ...
    const coverGooglePlaceIds =
      item.coverGooglePlaceIds === undefined ? [] : item.coverGooglePlaceIds;
    const validCoverGooglePlaceIds =
      Array.isArray(coverGooglePlaceIds) &&
      coverGooglePlaceIds.length <= 2 &&
      coverGooglePlaceIds.every(
        (placeId) =>
          typeof placeId === "string" &&
          /^[A-Za-z0-9_-]{10,200}$/.test(placeId),
      );
    if (
      !title ||
      !destination ||
    ...
    return {
      ...
      dayCount,
      itemCount,
      coverGooglePlaceIds: (coverGooglePlaceIds as string[]).map(
        (placeId) => placeId as GooglePlaceId,
      ),
    };
  });
...
function parseSavedTripItem(
  ...
  if (value.resolution === "UNRESOLVED") {
    if (
      value.googlePlaceId !== undefined ||
      value.latitude !== undefined ||
      value.longitude !== undefined ||
      value.placeAddress !== undefined ||
      value.placeCategory !== undefined ||
      value.placeResolvedAt !== undefined
    ) {
  ...
  if (
    ...
    longitude === null ||
    !googlePlaceId ||
    !isIsoTimestamp(value.placeResolvedAt) ||
    placeAddress === null ||
    placeCategory === null
  ) {
...
function parseSavedTripDay(
  ...
  if (
    !isRecord(value) ||
    !hasOnlyKeys(value, ["id", "dayNumber", "date", "summary", "items"]) ||
    !isUuid(value.id) ||
    value.dayNumber !== expectedDay ||
    !Array.isArray(value.items)
  ) {
  ...
  return {
    ...
    ...(summary === undefined ? {} : { summary }),
    items: value.items.map((item, index) =>
      parseSavedTripItem(item, index + 1),
    ),
  };
}
...
export function parseSavedTripDetail(value: unknown): SavedTripDetail | null {
  ...
  const destination = requiredString(value.destination, 120);
  const estimatedBudget =
    value.estimatedBudget === undefined || value.estimatedBudget === null
      ? null
  ...
  if (
    ...
    (estimatedBudget === null &&
      value.estimatedBudget !== undefined &&
  ...
  return {
    ...
    days: value.days.map((day, index) =>
      parseSavedTripDay(day, index + 1, value.startDate as string),
    ),
  };
}

export function validateResolvePlaceRequest(
  value: unknown,
): ResolvePlaceRequest {
  if (
    !isRecord(value) ||
    !hasOnlyKeys(value, ["itineraryItemId"]) ||
    !isUuid(value.itineraryItemId)
  ) {
...
export function parseResolvePlaceSuccess(
  value: unknown,
): ResolvePlaceSuccessEnvelope {
  if (
    !isRecord(value) ||
    ...
    (value.data.resolution !== "VERIFIED" &&
      value.data.resolution !== "VERIFIED_REFRESHED") ||
...
export function validateRouteRequest(value: unknown): RouteRequest {
  if (
    !isRecord(value) ||
    !hasOnlyKeys(value, ["profile", "coordinates"]) ||
    value.profile !== "driving" ||
    !Array.isArray(value.coordinates) ||
    value.coordinates.length < 2 ||
    value.coordinates.length > 25 ||
...
export function parseOsrmRoute(value: unknown): OsrmRouteTransport {
  ...
  if (
    !isRecord(route) ||
    finiteNumber(route.distance, 0, Number.MAX_SAFE_INTEGER) === null ||
    finiteNumber(route.duration, 0, Number.MAX_SAFE_INTEGER) === null ||
    ...
    route.geometry.coordinates.some(
      (point) =>
        !Array.isArray(point) ||
        point.length !== 2 ||
        finiteNumber(point[0], -180, 180) === null ||
        finiteNumber(point[1], -90, 90) === null,
    )
  ) {
    throw new ContractValidationError("OSRM response");
  }
...
export function validateWeatherRequest(value: unknown): WeatherRequest {
  if (
    !isRecord(value) ||
    !hasOnlyKeys(value, ["latitude", "longitude", "forecastDays"]) ||
    !isValidCoordinate({
      latitude: value.latitude,
      longitude: value.longitude,
    }) ||
    !Number.isInteger(value.forecastDays) ||
    (value.forecastDays as number) < 1 ||
    (value.forecastDays as number) > 16
  ) {
...
function parseNullableNumberArray(
  ...
  if (
    ...
    value.length !== length ||
    value.some(
      (item) => item !== null && finiteNumber(item, minimum, maximum) === null,
    )
  ) {
...
export function parseOpenMeteoForecast(value: unknown): OpenMeteoTransport {
  if (
    ...
    value.daily.time.length < 1 ||
    value.daily.time.length > 16 ||
  ...
  return {
    daily: {
      ...
      weather_code: parseNullableNumberArray(
        value.daily.weather_code,
        length,
        0,
        99,
      ),
      temperature_2m_max: parseNullableNumberArray(
        value.daily.temperature_2m_max,
        length,
        -100,
        100,
      ),
      temperature_2m_min: parseNullableNumberArray(
        value.daily.temperature_2m_min,
        length,
        -100,
        100,
      ),
      precipitation_probability_max: parseNullableNumberArray(
...
export function validateGetPlacePhotoRequest(
  value: unknown,
): GetPlacePhotoRequest {
  if (
    !isRecord(value) ||
    !hasOnlyKeys(value, ["googlePlaceId", "maxWidth"]) ||
...
export function parseGetPlacePhotoSuccess(value: unknown): {
  data: PlacePhoto;
} {
  if (
    !isRecord(value) ||
  ...
  if (value.data.diagnostic !== undefined && value.data.diagnostic !== null) {
    if (
      !isRecord(value.data.diagnostic) ||
  ...
  if (
    value.data.authorAttribution !== undefined &&
    value.data.authorAttribution !== null
  ) {
    ...
    authorAttribution = {
      displayName:
        typeof value.data.authorAttribution.displayName === "string"
          ? value.data.authorAttribution.displayName
          : undefined,
      uri:
        typeof value.data.authorAttribution.uri === "string"
          ? value.data.authorAttribution.uri
          : undefined,
      photoUri:
        typeof value.data.authorAttribution.photoUri === "string"
          ? value.data.authorAttribution.photoUri
          : undefined,
    };
  }
  return {
...
export function validateWikimediaImageRequest(
  value: unknown,
): WikimediaImageRequest {
  if (
    !isRecord(value) ||
    !hasOnlyKeys(value, ["kind", "googlePlaceId", "destination", "maxWidth"])
  ) {
  ...
  const maxWidth =
    value.maxWidth === undefined
      ? undefined
      : finiteNumber(value.maxWidth, 100, 1600);
...
export function parseWikimediaImageSuccess(value: unknown): {
  data: ResolvedImage;
} {
  if (
    !isRecord(value) ||
    ...
    (value.data.source !== "WIKIMEDIA_PLACE" &&
      value.data.source !== "DESTINATION_COVER")
  ) {
  ...
  if (value.data.attribution !== undefined) {
    if (
      !isRecord(value.data.attribution) ||
      !hasOnlyKeys(value.data.attribution, [
        "displayName",
        "sourceUrl",
        "license",
        "licenseUrl",
      ...
      (value.data.attribution.license !== undefined &&
        !requiredString(value.data.attribution.license, 160)) ||
      (value.data.attribution.licenseUrl !== undefined &&
        !requiredString(value.data.attribution.licenseUrl, 2_048))
    ) {
      throw new ContractValidationError("get-wikimedia-image response");
    }
    attribution = {
      ...
      ...(value.data.attribution.license === undefined
        ? {}
        : { license: value.data.attribution.license as string }),
      ...(value.data.attribution.licenseUrl === undefined
        ? {}
        : { licenseUrl: value.data.attribution.licenseUrl as string }),
    };
  }
  if (
    value.data.matchedEntity !== undefined &&
    !requiredString(value.data.matchedEntity, 500)
  ) {
    throw new ContractValidationError("get-wikimedia-image response");
  }
  const confidence =
    value.data.confidence === undefined
      ? undefined
      : finiteNumber(value.data.confidence, 0, 1);
  ...
  return {
    data: {
      ...
      ...(value.data.matchedEntity === undefined
        ? {}
        : { matchedEntity: value.data.matchedEntity as string }),
...
export function validateSavePlaceCommand(value: unknown): SavePlaceCommand {
  ...
  if (
    !googlePlaceId ||
    !/^[A-Za-z0-9_-]{10,200}$/.test(googlePlaceId) ||
    !name ||
    latitude === null ||
    longitude === null
  ) {
...
export function parseSavedPlacesPage(value: unknown): {
  ...
  if (value.nextCursor !== null && value.nextCursor !== undefined) {
    if (
      ...
      !hasOnlyKeys(value.nextCursor, ["createdAt", "id"])
    ) {
      throw new ContractValidationError("list_saved_places response");
    }
