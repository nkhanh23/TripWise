...
export class SupabaseAuthRepository implements AuthRepository {
  ...
  async signIn(email: string, password: string, signal?: AbortSignal): Promise<AuthenticatedSession> {
    return executeWithReliability(async (attemptSignal) => {
      const { data, error } = await raceWithAbort(
        this.client.auth.signInWithPassword({ email: email.trim().toLowerCase(), password }),
        attemptSignal,
      );
      if (error) throw mapAuthError(error);
  ...
  async signUp(
    ...
    return executeWithReliability(async (attemptSignal) => {
      const { data, error } = await raceWithAbort(this.client.auth.signUp({
  ...
  async deleteAccount(signal?: AbortSignal): Promise<void> {
    const { error } = await this.client.rpc('delete_user_account' as any)
  ...
  async signOut(signal?: AbortSignal): Promise<void> {
    return executeWithReliability(async (attemptSignal) => {
      const { error } = await raceWithAbort(this.client.auth.signOut(), attemptSignal);
      if (error) throw mapAuthError(error);
    }, authOperationPolicy, signal);
  }
