...
export class SupabaseTripGenerationRepository implements TripGenerationRepository {
  ...
  async generate(request: GenerateTripRequest, signal?: AbortSignal): Promise<GeneratedTrip> {
    ...
    return executeWithReliability(async (attemptSignal) => {
      ...
      if (error) throw mapGenerateTripError(await readFunctionErrorPayload(error));
      return parseGenerateTripSuccess(data).data;
    }, tripGenerationPolicy, signal);
  }
}
...
export class SupabaseTripPersistenceRepository implements TripPersistenceRepository {
  ...
  async persist(command: PersistTripCommand, signal?: AbortSignal): Promise<TripId> {
    ...
    }, idempotentPersistencePolicy, signal);
  }
}
...
export class SupabaseSavedTripsRepository implements SavedTripsRepository {
  ...
  async list(request: SavedTripsPageRequest = {}, signal?: AbortSignal): Promise<SavedTripsPage> {
  ...
  async getDetail(tripId: TripId, signal?: AbortSignal): Promise<SavedTripDetail | null> {
    return executeWithReliability(async (attemptSignal) => {
