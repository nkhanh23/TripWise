import type {
  GenerateTripErrorCode,
  PersistenceErrorCode,
  ResolvePlaceErrorCode,
} from "./contracts";
...
const persistenceCodes: readonly PersistenceErrorCode[] = [
  "TW001",
  "TW002",
  "TW003",
  "TW004",
...
export function mapGenerateTripError(value: unknown): IntegrationError {
  ...
  if (!generateCodes.includes(code as GenerateTripErrorCode))
  ...
  switch (code as GenerateTripErrorCode) {
    ...
    case "AI_TIMEOUT":
      return new IntegrationError("timeout");
    case "AI_UNAVAILABLE":
      return new IntegrationError("providerUnavailable");
...
export function mapPersistenceError(value: unknown): IntegrationError {
  ...
  if (!persistenceCodes.includes(rawCode as PersistenceErrorCode))
  ...
  switch (rawCode as PersistenceErrorCode) {
    ...
    case "TW002":
      return new IntegrationError("unauthorized");
    case "TW003":
      return new IntegrationError("forbidden");
    case "TW004":
...
export function mapResolvePlaceError(value: unknown): IntegrationError {
  const code = safeEnvelopeCode(value);
  if (!resolvePlaceCodes.includes(code as ResolvePlaceErrorCode))
    return new IntegrationError("unknown");
  switch (code as ResolvePlaceErrorCode) {
    case "PLACE_INPUT_INVALID":
      return new IntegrationError("invalidRequest");
    case "PLACE_NOT_FOUND":
      return new IntegrationError("notFound");
    case "PLACE_AMBIGUOUS":
      return new IntegrationError("ambiguousPlace");
    case "PLACE_PROVIDER_AUTH":
      return new IntegrationError("forbidden");
    case "PLACE_PROVIDER_RATE_LIMITED":
      return new IntegrationError("rateLimited");
    case "PLACE_PROVIDER_UNAVAILABLE":
      return new IntegrationError("providerUnavailable");
    case "PLACE_PERSISTENCE_FAILED":
      return new IntegrationError("persistenceFailed");
    case "UNAUTHORIZED":
      return new IntegrationError("unauthorized");
    case "INTERNAL_ERROR":
...
export async function readFunctionErrorPayload(
  value: unknown,
): Promise<unknown> {
...
export function mapPlacePhotoError(value: unknown): IntegrationError {
  ...
  switch (code) {
    ...
    case "PHOTO_NOT_FOUND":
      return new IntegrationError("notFound");
    case "PHOTO_PROVIDER_AUTH":
      return new IntegrationError("providerUnavailable");
    case "PHOTO_PROVIDER_RATE_LIMITED":
      return new IntegrationError("rateLimited");
    case "PHOTO_PROVIDER_UNAVAILABLE":
      return new IntegrationError("providerUnavailable", true);
    default:
...
export function mapWikimediaImageError(value: unknown): IntegrationError {
  ...
  switch (code) {
    ...
    case "FORBIDDEN":
      return new IntegrationError("forbidden");
    case "WIKIMEDIA_RATE_LIMITED":
      return new IntegrationError("rateLimited");
    case "WIKIMEDIA_UNAVAILABLE":
      return new IntegrationError("providerUnavailable", true);
    default:
      return mapUnknownTransportError(value);
  }
