...
export function generateLargeMockExplorePlaces(count = 50): ExplorePlace[] {
  ...
  for (let i = 1; i <= count; i++) {
    ...
    const category = isBase ? basePlace.category : categories[i % categories.length];

    places.push({
      id: `place_large_${i}`,
      name: isBase ? basePlace.name : `${basePlace.name} #${i}`,
      category,
      categoryLabel: isBase ? basePlace.categoryLabel : categoryLabels[category],
...
export const largeMockExplorePlaces: ExplorePlace[] = generateLargeMockExplorePlaces(50);
