...
export function RoutePreviewScreen({
  ...
  const destinationName =
    route?.params?.destinationName ?? t("route.destination");
  ...
  const [selectedMode, setSelectedMode] =
    useState<TransportMode>(effectiveInitialMode);
  ...
  const [calculatedRoute, setCalculatedRoute] = useState<MockRouteData | null>(
    () =>
      customRoute ??
      (isFixture && initialStatus === "ready"
        ? getMockRoute(destinationId, effectiveInitialMode)
        : null),
  );
  ...
  const fetchRealRoute = useCallback(
    (mode: TransportMode) => {
      ...
      setStatus("loading");
      void new OsrmRouteRepository()
        .getRoute(
          { profile: "driving", coordinates: realCoordinates },
          controller.signal,
        )
        .then((result) => {
        ...
        {realRoute && status === "ready" ? (
          <View style={styles.realSummary}>
            ...
            <Text style={{ color: colors.text.secondary }}>
              {(realRoute.distanceMeters / 1000).toFixed(1)} km •{" "}
              {Math.ceil(realRoute.durationSeconds / 60)} min
            </Text>
          </View>
        ) : null}
      ...
      >
        ...
        {status === "unavailable" ? (
          <RouteUnavailableState
            onBack={handleBack}
            onSwitchTransport={handleSelectMode}
          />
        ) : null}

        {/* Ready Route Content */}
        {status === "ready" && hasRealRouteRequest ? listHeader : null}
      ...
      {status === "ready" && (currentRoute || realRoute) ? (
        ...
        >
          <Pressable
            ...
            style={({ pressed }) => [
              styles.ctaButton,
              { backgroundColor: colors.brand.primary, opacity: 0.5 },
