...
type CombinedNavProp = NativeStackNavigationProp<RootStackParamList> & BottomTabNavigationProp<MainTabParamList>;
type Props = { onNavigateSettings?: () => void };

export const ProfileScreen = memo(function ProfileScreen({ onNavigateSettings }: Props) {
  ...
  const {
    profile, profileStatus, refreshProfile, tripsCount, savedCount, countriesCount,
  } = useProfile();
  const [activeAction, setActiveAction] = useState<DestructiveActionType>(null);
  const [signingOut, setSigningOut] = useState(false);
  const [actionError, setActionError] = useState<string | null>(null);

  const handleEditPress = useCallback(() => navigation.navigate('EditProfile'), [navigation]);
  const handleNavigateTrips = useCallback(() => navigation.navigate('Trips'), [navigation]);
  const handleNavigateSaved = useCallback(() => navigation.navigate('Saved'), [navigation]);
  const handleNavigateExplore = useCallback(() => navigation.navigate('Explore'), [navigation]);
  ...
  const menuSections = useMemo<ProfileMenuSectionData[]>(() => [
    {
      ...
      items: [
        { id: 'my_trips', label: t('profile.menu.myTrips'), iconName: 'flight', onPress: handleNavigateTrips },
        { id: 'saved_places', label: t('profile.menu.savedPlaces'), iconName: 'bookmark', onPress: handleNavigateSaved },
      ],
    },
    {
      title: t('profile.sections.account'),
      items: [
        { id: 'personal_info', label: t('profile.menu.personalInfo'), iconName: 'badge', onPress: handleEditPress },
        { id: 'settings', label: t('profile.menu.settings'), iconName: 'settings', onPress: handleOpenSettings },
      ],
    },
    {
  ...
  return (
    <View style={[styles.screen, { backgroundColor: colors.background.canvas, paddingTop: insets.top }]}>
      <View style={[
        ...
        <Text style={[styles.brandText, { color: colors.brand.primary }]}>TripWise</Text>
        <Pressable
      ...
      <ScrollView
        ...
        <View style={styles.titleContainer}>
          <Text style={[styles.pageTitle, { color: colors.text.primary }]}>{t('profile.title')}</Text>
        </View>

        {profileStatus === 'loading' ? (
          <View style={styles.profileState}>
            <ActivityIndicator color={colors.brand.primary} />
            <Text style={[styles.profileStateText, { color: colors.text.secondary }]}>{t('profile.loading')}</Text>
          </View>
        ) : null}

        {profileStatus === 'error' || profileStatus === 'absent' ? (
          <View accessibilityRole="alert" style={styles.profileState}>
            <Text style={[styles.profileStateText, { color: colors.text.secondary }]}>
              {t(profileStatus === 'error' ? 'profile.loadError' : 'profile.absent')}
            </Text>
            <Pressable accessibilityLabel={t('profile.retry')} accessibilityRole="button" onPress={() => void refreshProfile()}>
              <Text style={{ color: colors.brand.primary }}>{t('profile.retry')}</Text>
            </Pressable>
          </View>
        ) : null}

        {actionError ? (
          <Text accessibilityRole="alert" style={[styles.actionError, { color: colors.state.error }]}>{actionError}</Text>
        ) : null}
        ...
        {menuSections.map((section) => (
          <ProfileMenuSection items={section.items} key={section.title} title={section.title} />
        ))}
        ...
        <View style={styles.signOutSection}>
          <Pressable
            ...
            <Text style={[styles.signOutText, { color: colors.state.error }]}>{t('profile.menu.signOut')}</Text>
          </Pressable>
        </View>
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  brandText: { fontSize: typography.body, fontWeight: typography.fontWeight.bold, letterSpacing: 0.5 },
  scrollContent: { paddingBottom: spacing.xxxl },
  titleContainer: { paddingHorizontal: spacing.lg, paddingTop: spacing.md },
  pageTitle: { fontSize: typography.title, fontWeight: typography.fontWeight.bold },
  profileState: { alignItems: 'center', gap: spacing.sm, paddingHorizontal: spacing.lg, paddingVertical: spacing.xl },
  profileStateText: { fontSize: typography.bodySmall, textAlign: 'center' },
  actionError: { fontSize: typography.bodySmall, paddingHorizontal: spacing.lg, paddingTop: spacing.md, textAlign: 'center' },
  signOutSection: { alignItems: 'center', marginTop: spacing.xxl, paddingBottom: spacing.lg },
  signOutButton: {
    alignItems: 'center', flexDirection: 'row', gap: spacing.sm,
    paddingHorizontal: spacing.lg, paddingVertical: spacing.md,
  },
  signOutText: { fontSize: typography.body, fontWeight: typography.fontWeight.bold },
  pressed: { opacity: 0.7 },
});
