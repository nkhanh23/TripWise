...
export const EditProfileScreen = memo(function EditProfileScreen({
  ...
  const handleChangePhoto = useCallback(() => {
    const currentIndex = avatarUrl
      ? PROFILE_AVATAR_OPTIONS.indexOf(
          avatarUrl as (typeof PROFILE_AVATAR_OPTIONS)[number],
        )
      : -1;
    setAvatarUrl(
      PROFILE_AVATAR_OPTIONS[
        (currentIndex + 1) % PROFILE_AVATAR_OPTIONS.length
      ],
    );
  }, [avatarUrl]);
  ...
  if (!profile) {
    return (
      <View
        style={[styles.centered, { backgroundColor: colors.background.canvas }]}
      >
          ...
          >
            ...
            {saveError ? (
              <Text
                ...
                style={[styles.errorText, { color: colors.state.error }]}
              >
                {saveError}
              </Text>
            ) : null}
