...
export type PlaceResolutionStatus =
  "UNRESOLVED_IDLE" | "RESOLVING" | "VERIFIED" | "ERROR";

export function usePlaceResolution(
  repository?: PlaceResolutionRepository,
  onRefresh?: () => Promise<boolean>,
) {
  const [statuses, setStatuses] = useState<
    Record<string, PlaceResolutionStatus>
  >({});
  ...
  const resolve = useCallback(
    async (itemId: string) => {
      ...
      let typedId;
      try {
        typedId = asItineraryItemId(itemId);
      } catch {
        return false;
      }
      ...
      try {
        const result = await repository.resolve(
          { itineraryItemId: typedId },
          controller.signal,
        );
        const refreshed = await onRefresh?.();
        const verified =
          result.resolution.startsWith("VERIFIED") && refreshed !== false;
        setStatuses((current) => ({
          ...current,
          [itemId]: verified ? "VERIFIED" : "ERROR",
        }));
        return verified;
      } catch (error) {
