import type { SavedTripDetail, SavedTripSummary } from '../../integration/contracts';
...
export function mapSavedTripSummaryToTripSummary(summary: SavedTripSummary): TripSummary {
...
export function mapSavedTripPageToSections(items: SavedTripSummary[]): TripSectionData[] {
  const mapped = items.map(mapSavedTripSummaryToTripSummary);
  return [
    { type: 'upcoming', title: 'Upcoming', iconName: 'flight-takeoff', iconColor: '', data: mapped.filter((item) => item.status === 'upcoming') },
    { type: 'past', title: 'Past Trips', iconName: 'history', iconColor: '', data: mapped.filter((item) => item.status === 'past') },
  ];
}

export function mapSavedTripDetailToTripDetailData(detail: SavedTripDetail): TripDetailData {
  return {
    id: detail.id,
    title: detail.title,
