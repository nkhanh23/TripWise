...
export function useTripPlacePhotos(
        ...
        ? [
            ...new Set(
              tripData.days
                ...
                .filter(
                  (item) =>
                    item.resolution === "VERIFIED" && item.googlePlaceId,
                )
  ...
  const resolutionIdentity = useMemo(
    () =>
      JSON.stringify(
        tripData
          ? ({
              ...
              tripId: tripData.id,
              verifiedIds,
            } satisfies TripImageResolutionInput)
          : null,
      ),
    [tripData, verifiedIds],
  );

  useEffect(() => {
    const input = JSON.parse(
      resolutionIdentity,
    ) as TripImageResolutionInput | null;
    ...
    async function loadItems(): Promise<void> {
      ...
      const loadNext = async (): Promise<void> => {
        while (!controller.signal.aborted) {
          ...
          const image = await placeRepository
            .getPlaceImage({ googlePlaceId, maxWidth: 600 }, controller.signal)
            .catch(() => null);
