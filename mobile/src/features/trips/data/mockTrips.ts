...
export const mockUpcomingTrips: TripSummary[] = [
  {
    ...
    travelers: [
      { id: 'u1', name: 'John Smith', initials: 'JS', colorVariant: 'secondary' },
...
export function generateLargeMockTrips(count = 20): TripSectionData[] {
  ...
  for (let i = 1; i <= count; i++) {
    ...
    const trip: TripSummary = {
      ...
      dateLabel: isUpcoming ? `Nov ${i} - Nov ${i + 7}, 2026` : `Mar 2024 (Trip #${i})`,
      status: isUpcoming ? 'upcoming' : 'past',
      statusBadgeText: isUpcoming ? (i % 2 === 0 ? 'Confirmed' : 'Planning') : undefined,
      statusBadgeVariant: isUpcoming ? (i % 2 === 0 ? 'primary' : 'surface') : undefined,
      travelers: isUpcoming
        ? [{ id: `u_${i}`, name: 'Alex Morgan', initials: 'AM', colorVariant: 'secondary' }]
        : undefined,
