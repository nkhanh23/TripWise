...
function isGeneratedItem(value: unknown): boolean {
  return isRecord(value)
    ...
    && (value.estimatedCost === undefined || (typeof value.estimatedCost === 'number' && Number.isFinite(value.estimatedCost) && value.estimatedCost >= 0));
}
...
export function parseGenerateTripResponse(value: unknown): GeneratedTrip | null {
  if (!isRecord(value) || !isRecord(value.data)) {
    return null;
  }
  const trip = value.data;
  if (!isString(trip.title)
