...
function isKnownErrorCode(value: unknown): value is GenerateTripErrorCode {
  return (
    typeof value === "string" &&
    knownErrorCodes.includes(value as GenerateTripErrorCode)
  );
}

export function mapGenerateTripErrorPayload(
  value: unknown,
): GenerateTripClientError {
  ...
  return new GenerateTripClientError(
    "INTERNAL_ERROR",
...
async function mapInvokeError(
  error: unknown,
): Promise<GenerateTripClientError> {
  const context =
  ...
  if (
    ...
    try {
      const response = (
        context as { clone: () => { json: () => Promise<unknown> } }
      ).clone();
...
export async function generateTrip(
  ...
  if (!trip) {
    throw new GenerateTripClientError(
      "AI_INVALID_RESPONSE",
      "Dịch vụ AI trả về lịch trình không hợp lệ.",
    );
  }
  return trip;
}
