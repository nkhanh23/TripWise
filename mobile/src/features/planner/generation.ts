...
export function useTripGeneration(repository?: TripGenerationRepository) {
  ...
  const generate = useCallback(
    async (intent: CreateTripWizardState) => {
      ...
      try {
        const activeRepository =
          repository ??
          new SupabaseTripGenerationRepository(
            (await import("../../lib/supabase/client")).supabase,
          );
        const generated = await activeRepository.generate(
          mapWizardStateToGenerateTripRequest(intent),
          controller.signal,
        );
