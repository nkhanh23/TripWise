...
export function useTripPersistence(repository?: TripPersistenceRepository) {
  ...
  const save = useCallback(
    async (
      ...
      previewRef.current = { preview, title, budgetConfig };
      ...
      try {
        const graph = mapPlannerPreviewToPersistenceGraph(preview, title, budgetConfig);
  ...
  const retry = useCallback(async () => {
    ...
    return save(
      ...
      previewRef.current.budgetConfig,
    );
  }, [save]);
