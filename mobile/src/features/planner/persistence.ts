...
import { IdempotencyKeyFactory, SaveIntent } from '../../integration/idempotency';
...
import { mapPlannerPreviewToPersistenceGraph, type PlannerGeneratedPreview } from './generationContracts';
...
export function useTripPersistence(repository?: TripPersistenceRepository) {
  const [state, setState] = useState<TripPersistenceState>({ status: 'idle', tripId: null, error: null });
  const repositoryRef = useRef(repository);
  const intentRef = useRef<SaveIntent | null>(null);
  const mountedRef = useRef(true);
  const controllerRef = useRef<AbortController | null>(null);
  const previewRef = useRef<{ preview: PlannerGeneratedPreview; title?: string | null } | null>(null);
  ...
  const save = useCallback(async (preview: PlannerGeneratedPreview, title?: string | null) => {
    if (controllerRef.current) return null;
    const controller = new AbortController();
    controllerRef.current = controller;
    previewRef.current = { preview, title };
    const intent = intentRef.current ?? new IdempotencyKeyFactory().createSaveIntent();
    intentRef.current = intent;
    setState({ status: 'saving', tripId: null, error: null });
    try {
      const activeRepository = repositoryRef.current ?? new SupabaseTripPersistenceRepository(
      ...
      if (mountedRef.current) setState({ status: 'success', tripId, error: null });
      return tripId;
    } catch (error) {
